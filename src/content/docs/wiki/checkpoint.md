...
[데이터 증강](/wiki/data-augmentation/) 및 [합성 데이터](/wiki/synthetic-data/) 개념을 먼저 이해하면 계산 위치와 역할을 구분하기 쉽다. 이 선행 관계를 기준으로 어느 단계에서 값이 만들어지고 다음 구성 요소로 어떻게 전달되는지 추적하면, 비슷한 용어를 기능 이름만으로 혼동하는 일을 줄일 수 있다.
...
2. **입력과 조건 확인:** [데이터 증강](/wiki/data-augmentation/), [합성 데이터](/wiki/synthetic-data/)의 정의와 입력 조건을 먼저 확인한다.
...
2. [Checkpointing — Wikipedia](https://en.wikipedia.org/wiki/Checkpointing) — encyclopedia
