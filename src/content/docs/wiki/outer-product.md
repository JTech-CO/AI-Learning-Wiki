---
title: "외적곱 Outer Product"
description: "한 벡터의 각 성분과 다른 벡터의 각 성분을 모두 곱해 행렬을 만드는 연산이다."
tableOfContents: { minHeadingLevel: 2, maxHeadingLevel: 4 }
---

<p class="wiki-lead">한 벡터의 각 성분과 다른 벡터의 각 성분을 모두 곱해 행렬을 만드는 연산이다.</p>
...
한 벡터의 각 성분과 다른 벡터의 각 성분을 모두 곱해 행렬을 만드는 연산이다.

열벡터 a와 행벡터 b의 곱 a bᵀ를 계산해 각 원소가 a_i b_j가 되게 한다. 이 표제어는 Outer Product이라는 영문 용어와 함께 사용되며, 구현 이름보다 입력·처리·출력과 성립 조건을 기준으로 이해해야 한다. 핵심 정의를 실제 시스템에 적용할 때는 모델이나 도구의 버전, 데이터 범위와 평가 단위를 함께 적어 같은 이름 아래 다른 동작이 섞이지 않게 한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-2">[2]</a></div>

### 배경과 설명 범위

한 벡터의 각 성분과 다른 벡터의 각 성분을 모두 곱해 행렬을 만드는 연산이다. 설명 범위에는 개념의 역사적 배경뿐 아니라 현재의 계산 절차와 운영 경계가 포함된다. 내적은 같은 위치의 성분을 합하지만 외적은 모든 성분 쌍의 상호작용을 보존한다.
...
열벡터 a와 행벡터 b의 곱 a bᵀ를 계산해 각 원소가 a_i b_j가 되게 한다.
...
길이 m과 n의 두 벡터, 축 방향과 결과 m×n 행렬이 핵심 요소다.
...
랭크 1 행렬 구성, 공분산 계산, 저랭크 근사와 신경망의 기울기 표현에 사용한다.
...
내적과 표기가 혼동되기 쉽고 큰 벡터에서는 결과 행렬의 메모리 사용량이 빠르게 증가한다.

외적곱을 수학적으로 사용할 때는 기호의 정의역과 공역, 배열의 축과 차원, 단위와 정규화 조건을 먼저 적는다. 식이 맞더라도 차원이 호환되지 않거나 수치 정밀도와 경계 조건이 다르면 구현 결과가 달라진다. 손으로 계산한 작은 예제, 라이브러리 결과와 오차 허용 범위를 함께 비교해 의미와 구현을 연결한다. 평균값만으로 한계를 숨기지 않도록 드문 입력, 분포 변화, 악의적 조작과 자원 고갈 조건을 따로 시험한다. 알려진 실패를 탐지할 지표와 자동 중단선, 안전한 대체 경로, 사람이 검토할 사건을 사전에 정한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-2">[2]</a> <a href="#reference-3">[3]</a></div>

### 관련 개념과의 구분

내적은 같은 위치의 성분을 합하지만 외적은 모든 성분 쌍의 상호작용을 보존한다.
...
길이 2와 3인 벡터의 외적을 손으로 계산하고 라이브러리 브로드캐스팅 결과가 2×3인지 확인한다.
...
1. **목적과 경계 정의:** 외적곱이 해결할 문제와 해결하지 않을 문제를 각각 한 문장으로 적는다.
...
한 벡터의 각 성분과 다른 벡터의 각 성분을 모두 곱해 행렬을 만드는 연산이다. 열벡터 a와 행벡터 b의 곱 a bᵀ를 계산해 각 원소가 a_i b_j가 되게 한다. 길이 m과 n의 두 벡터, 축 방향과 결과 m×n 행렬이 핵심 요소다. 이 세 문장을 입력·변환·출력의 순서로 다시 써 보고, 랭크 1 행렬 구성, 공분산 계산, 저랭크 근사와 신경망의 기울기 표현에 사용한다.라는 적용 범위에서 내적과 표기가 혼동되기 쉽고 큰 벡터에서는 결과 행렬의 메모리 사용량이 빠르게 증가한다.라는 한계가 어떤 관측 지표로 나타나는지 정의한다. 길이 2와 3인 벡터의 외적을 손으로 계산하고 라이브러리 브로드캐스팅 결과가 2×3인지 확인한다. 예시를 정상·경계·실패 사례로 나누고 각 사례의 기대 결과와 허용 오차, 중단 조건을 표로 만든다. 마지막으로 관련 문서와의 차이를 용어가 아니라 데이터 요구량, 계산 절차, 출력 형식과 실패 비용으로 비교한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-2">[2]</a> <a href="#reference-3">[3]</a></div>

### 학습 체크

- 외적곱의 입력, 처리와 출력을 각각 설명할 수 있는가?
...
<span id="reference-4"></span>4. [Outer product — Wikipedia](https://en.wikipedia.org/wiki/Outer_product) — encyclopedia
