...
description: "이미지나 영상에서 관심 객체의 종류와 위치를 경계 상자 같은 공간 좌표로 함께 예측하는 컴퓨터 비전 과제다."
tableOfContents: { minHeadingLevel: 2, maxHeadingLevel: 3 }
---

<p class="wiki-lead">이미지나 영상에서 관심 객체의 종류와 위치를 경계 상자 같은 공간 좌표로 함께 예측하는 컴퓨터 비전 과제다.</p>

<div class="wiki-document-meta">분류: [멀티모달 AI](/category/multimodal/) · 문서 상태: 문장 단위 근거 검토 완료 · 최근 검토: 2026-07-13</div>

## 개요와 핵심 정의

이미지나 영상에서 관심 객체의 종류와 위치를 경계 상자 같은 공간 좌표로 함께 예측하는 컴퓨터 비전 과제다.

백본이 시각 특징을 추출하고 탐지 헤드가 후보 위치와 클래스 점수를 예측한 뒤 중복 상자를 억제한다. 이 표제어는 Object Detection이라는 영문 용어와 함께 사용되며, 구현 이름보다 입력·처리·출력과 성립 조건을 기준으로 이해해야 한다. 핵심 정의를 실제 시스템에 적용할 때는 모델이나 도구의 버전, 데이터 범위와 평가 단위를 함께 적어 같은 이름 아래 다른 동작이 섞이지 않게 한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a></div>

## 배경과 설명 범위

이미지나 영상에서 관심 객체의 종류와 위치를 경계 상자 같은 공간 좌표로 함께 예측하는 컴퓨터 비전 과제다. 설명 범위에는 개념의 역사적 배경뿐 아니라 현재의 계산 절차와 운영 경계가 포함된다. 이미지 분류가 이미지 전체의 레이블을 예측한다면 객체 탐지는 여러 객체의 레이블과 위치를 함께 출력한다.
...
백본이 시각 특징을 추출하고 탐지 헤드가 후보 위치와 클래스 점수를 예측한 뒤 중복 상자를 억제한다.
...
입력 전처리, 특징 추출기, 후보 또는 앵커, 분류·회귀 헤드, 후처리와 좌표계가 필요하다.
...
자율주행, 제조 검사, 문서 이해, 보안 영상과 이미지 검색에 사용한다.
...
작은 객체, 가림, 조명 변화와 도메인 이동에 취약하며 경계 상자 정답의 모호성도 평가에 영향을 준다.

객체 탐지는 서로 다른 감각 표현과 정답 단위를 연결하므로 입력 전처리, 정렬 방식, 출력 좌표계와 평가 기준을 함께 정의해야 한다. 텍스트와 이미지의 상관관계가 실제 근거인지 데이터 편향인지 구분하기 위해 반사실적 사례와 영역별 성능을 확인한다. 개인정보, 저작권과 접근성 요구도 모델 정확도와 별개의 운영 조건으로 다룬다. 평균값만으로 한계를 숨기지 않도록 드문 입력, 분포 변화, 악의적 조작과 자원 고갈 조건을 따로 시험한다. 알려진 실패를 탐지할 지표와 자동 중단선, 안전한 대체 경로, 사람이 검토할 사건을 사전에 정한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

## 관련 개념과의 구분

이미지 분류가 이미지 전체의 레이블을 예측한다면 객체 탐지는 여러 객체의 레이블과 위치를 함께 출력한다.
...
창고 사진에서 상자와 지게차를 탐지하고 IoU 임계값별 정밀도·재현율과 작은 객체 실패 사례를 확인한다.
...
1. **목적과 경계 정의:** 객체 탐지가 해결할 문제와 해결하지 않을 문제를 각각 한 문장으로 적는다.
...
이미지나 영상에서 관심 객체의 종류와 위치를 경계 상자 같은 공간 좌표로 함께 예측하는 컴퓨터 비전 과제다. 백본이 시각 특징을 추출하고 탐지 헤드가 후보 위치와 클래스 점수를 예측한 뒤 중복 상자를 억제한다. 입력 전처리, 특징 추출기, 후보 또는 앵커, 분류·회귀 헤드, 후처리와 좌표계가 필요하다. 이 세 문장을 입력·변환·출력의 순서로 다시 써 보고, 자율주행, 제조 검사, 문서 이해, 보안 영상과 이미지 검색에 사용한다.라는 적용 범위에서 작은 객체, 가림, 조명 변화와 도메인 이동에 취약하며 경계 상자 정답의 모호성도 평가에 영향을 준다.라는 한계가 어떤 관측 지표로 나타나는지 정의한다. 창고 사진에서 상자와 지게차를 탐지하고 IoU 임계값별 정밀도·재현율과 작은 객체 실패 사례를 확인한다. 예시를 정상·경계·실패 사례로 나누고 각 사례의 기대 결과와 허용 오차, 중단 조건을 표로 만든다. 마지막으로 관련 문서와의 차이를 용어가 아니라 데이터 요구량, 계산 절차, 출력 형식과 실패 비용으로 비교한다.
...
- 객체 탐지의 입력, 처리와 출력을 각각 설명할 수 있는가?
...
<span id="reference-5"></span>5. [객체 탐지 — 한국어 위키백과](https://ko.wikipedia.org/wiki/%EA%B0%9D%EC%B2%B4_%ED%83%90%EC%A7%80) — encyclopedia
