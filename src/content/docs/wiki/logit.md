...
확률 변환 전에 모델이 각 후보 토큰에 부여하는 정규화되지 않은 점수다.
...
로짓은 소프트맥스에 들어가기 전의 정규화되지 않은 점수로, 값의 차이가 클래스나 토큰의 상대 확률을 결정한다.
...
지연 시간, 처리량, 메모리, 비용과 출력 품질의 균형을 맞추는 데 사용한다. ‘로짓’ 개념을 도입할 때는 기대 효과를 품질, 지연 시간, 처리량, 메모리, 비용, 안전성 중 측정 가능한 항목으로 바꾼다. 그다음 단순한 기준선과 비교해 개선 폭과 추가 복잡도를 함께 기록한다.
