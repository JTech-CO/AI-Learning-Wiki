...
SDK는 특정 API를 쉽게 호출하도록 인증, 요청 객체, 응답 타입, 재시도 같은 기능을 언어별 라이브러리로 묶는다.
...
최소 요청 예제에는 인증 방식, 필수 필드, 정상 응답, 오류 응답과 시간 초과 처리를 함께 담아야 계약의 경계가 보인다. ‘SDK’를 적용하는 경우에는 SDK는 특정 API를 쉽게 호출하도록 인증, 요청 객체, 응답 타입, 재시도 같은 기능을 언어별 라이브러리로 묶는다.
