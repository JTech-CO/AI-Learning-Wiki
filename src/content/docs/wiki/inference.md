...
**용량 계산 사례:** 한 요청의 평균만으로 GPU 수를 정하지 말고 입력·출력 길이의 상위 백분위와 동시 도착을 부하에 반영한다. 목표 트래픽을 단계적으로 올리면서 첫 토큰 지연과 완료 지연의 상위 백분위가 문턱을 넘는 지점을 찾고, 그보다 여유 있는 용량을 운영 기준으로 둔다. KV 캐시 부족으로 선점과 재계산이 급증하는 구간은 이론상 토큰 처리량이 높아도 안정 용량에서 제외한다.
