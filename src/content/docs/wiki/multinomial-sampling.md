---
title: "다항 표본추출 Multinomial Sampling"
description: "각 토큰의 확률을 범주형 분포로 보고 그 분포에서 다음 토큰 하나를 무작위로 추출하는 생성 방법이다."
tableOfContents: { minHeadingLevel: 2, maxHeadingLevel: 4 }
---

<p class="wiki-lead">각 토큰의 확률을 범주형 분포로 보고 그 분포에서 다음 토큰 하나를 무작위로 추출하는 생성 방법이다.</p>
...
각 토큰의 확률을 범주형 분포로 보고 그 분포에서 다음 토큰 하나를 무작위로 추출하는 생성 방법이다.

로짓을 온도와 필터로 변환해 정규화한 뒤 누적 확률과 난수를 사용해 한 토큰을 선택하고 문맥에 추가한다. 이 표제어는 Multinomial Sampling이라는 영문 용어와 함께 사용되며, 구현 이름보다 입력·처리·출력과 성립 조건을 기준으로 이해해야 한다. 핵심 정의를 실제 시스템에 적용할 때는 모델이나 도구의 버전, 데이터 범위와 평가 단위를 함께 적어 같은 이름 아래 다른 동작이 섞이지 않게 한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a></div>

### 배경과 설명 범위

각 토큰의 확률을 범주형 분포로 보고 그 분포에서 다음 토큰 하나를 무작위로 추출하는 생성 방법이다. 설명 범위에는 개념의 역사적 배경뿐 아니라 현재의 계산 절차와 운영 경계가 포함된다. 그리디 디코딩은 항상 최댓값을 선택하지만 다항 표본추출은 전체 남은 확률 질량에 따라 선택한다.
...
로짓을 온도와 필터로 변환해 정규화한 뒤 누적 확률과 난수를 사용해 한 토큰을 선택하고 문맥에 추가한다.
...
로짓, 소프트맥스 확률, 난수 생성기, 시드, 필터링 규칙과 종료 토큰이 필요하다.
...
언어 모델에서 다양한 문장을 생성하고 동일 문맥에서 여러 후보를 탐색하는 데 사용한다.
...
낮은 확률의 부적절한 토큰도 선택될 수 있고 시드와 부동소수점 구현에 따라 재현성이 달라진다.

다항 표본추출을 추론 과정에서 검토할 때는 로짓 변환, 후보 선택, 난수 사용, 종료 조건과 반복 가능한 설정을 나누어 본다. 품질은 한 번의 출력이 아니라 여러 시드와 길이, 입력 유형에서 측정해야 하며 지연시간과 메모리 사용도 함께 비교한다. 샘플링 설정과 모델 버전, 토크나이저가 바뀌면 같은 프롬프트라도 분포가 달라질 수 있다. 평균값만으로 한계를 숨기지 않도록 드문 입력, 분포 변화, 악의적 조작과 자원 고갈 조건을 따로 시험한다. 알려진 실패를 탐지할 지표와 자동 중단선, 안전한 대체 경로, 사람이 검토할 사건을 사전에 정한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

### 관련 개념과의 구분

그리디 디코딩은 항상 최댓값을 선택하지만 다항 표본추출은 전체 남은 확률 질량에 따라 선택한다.
...
고정 프롬프트와 시드에서 확률 분포를 기록하고 여러 번 추출해 경험적 빈도가 목표 확률과 가까운지 확인한다.
...
1. **목적과 경계 정의:** 다항 표본추출이 해결할 문제와 해결하지 않을 문제를 각각 한 문장으로 적는다.
...
각 토큰의 확률을 범주형 분포로 보고 그 분포에서 다음 토큰 하나를 무작위로 추출하는 생성 방법이다. 로짓을 온도와 필터로 변환해 정규화한 뒤 누적 확률과 난수를 사용해 한 토큰을 선택하고 문맥에 추가한다. 로짓, 소프트맥스 확률, 난수 생성기, 시드, 필터링 규칙과 종료 토큰이 필요하다. 이 세 문장을 입력·변환·출력의 순서로 다시 써 보고, 언어 모델에서 다양한 문장을 생성하고 동일 문맥에서 여러 후보를 탐색하는 데 사용한다.라는 적용 범위에서 낮은 확률의 부적절한 토큰도 선택될 수 있고 시드와 부동소수점 구현에 따라 재현성이 달라진다.라는 한계가 어떤 관측 지표로 나타나는지 정의한다. 고정 프롬프트와 시드에서 확률 분포를 기록하고 여러 번 추출해 경험적 빈도가 목표 확률과 가까운지 확인한다. 예시를 정상·경계·실패 사례로 나누고 각 사례의 기대 결과와 허용 오차, 중단 조건을 표로 만든다. 마지막으로 관련 문서와의 차이를 용어가 아니라 데이터 요구량, 계산 절차, 출력 형식과 실패 비용으로 비교한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

### 학습 체크

- 다항 표본추출의 입력, 처리와 출력을 각각 설명할 수 있는가?
