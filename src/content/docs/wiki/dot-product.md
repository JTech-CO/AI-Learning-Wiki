...
description: "같은 길이의 두 벡터에서 대응 성분의 곱을 더해 하나의 스칼라를 만드는 연산이다."
tableOfContents: { minHeadingLevel: 2, maxHeadingLevel: 3 }
---

<p class="wiki-lead">같은 길이의 두 벡터에서 대응 성분의 곱을 더해 하나의 스칼라를 만드는 연산이다.</p>

<div class="wiki-document-meta">분류: [수학·통계 기초](/category/mathematics/) · 문서 상태: 문장 단위 근거 검토 완료 · 최근 검토: 2026-07-13</div>

## 개요와 핵심 정의

같은 길이의 두 벡터에서 대응 성분의 곱을 더해 하나의 스칼라를 만드는 연산이다.

성분별 곱의 합으로 계산하며 유클리드 공간에서는 벡터 크기와 사이각의 코사인을 곱한 값과 같다. 이 표제어는 Dot Product이라는 영문 용어와 함께 사용되며, 구현 이름보다 입력·처리·출력과 성립 조건을 기준으로 이해해야 한다. 핵심 정의를 실제 시스템에 적용할 때는 모델이나 도구의 버전, 데이터 범위와 평가 단위를 함께 적어 같은 이름 아래 다른 동작이 섞이지 않게 한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-2">[2]</a></div>

## 배경과 설명 범위

같은 길이의 두 벡터에서 대응 성분의 곱을 더해 하나의 스칼라를 만드는 연산이다. 설명 범위에는 개념의 역사적 배경뿐 아니라 현재의 계산 절차와 운영 경계가 포함된다. 외적이 두 벡터에서 행렬을 만든다면 내적은 대응 성분을 합해 스칼라를 만든다.
...
성분별 곱의 합으로 계산하며 유클리드 공간에서는 벡터 크기와 사이각의 코사인을 곱한 값과 같다.
...
두 입력 벡터, 동일한 차원, 성분 곱, 합산과 선택한 수 체계가 필요하다.
...
유사도 계산, 투영, 선형 계층, 어텐션 점수와 에너지 계산에 사용한다.
...
벡터 크기에 영향을 받으므로 방향 유사도만 비교할 때는 정규화가 필요하고 복소수에서는 켤레 규칙을 확인해야 한다.

내적을 수학적으로 사용할 때는 기호의 정의역과 공역, 배열의 축과 차원, 단위와 정규화 조건을 먼저 적는다. 식이 맞더라도 차원이 호환되지 않거나 수치 정밀도와 경계 조건이 다르면 구현 결과가 달라진다. 손으로 계산한 작은 예제, 라이브러리 결과와 오차 허용 범위를 함께 비교해 의미와 구현을 연결한다. 평균값만으로 한계를 숨기지 않도록 드문 입력, 분포 변화, 악의적 조작과 자원 고갈 조건을 따로 시험한다. 알려진 실패를 탐지할 지표와 자동 중단선, 안전한 대체 경로, 사람이 검토할 사건을 사전에 정한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-2">[2]</a> <a href="#reference-3">[3]</a></div>

## 관련 개념과의 구분

외적이 두 벡터에서 행렬을 만든다면 내적은 대응 성분을 합해 스칼라를 만든다.
...
두 문서 임베딩의 내적과 코사인 유사도를 계산해 벡터 정규화 전후 순위가 어떻게 달라지는지 확인한다.
...
1. **목적과 경계 정의:** 내적이 해결할 문제와 해결하지 않을 문제를 각각 한 문장으로 적는다.
...
같은 길이의 두 벡터에서 대응 성분의 곱을 더해 하나의 스칼라를 만드는 연산이다. 성분별 곱의 합으로 계산하며 유클리드 공간에서는 벡터 크기와 사이각의 코사인을 곱한 값과 같다. 두 입력 벡터, 동일한 차원, 성분 곱, 합산과 선택한 수 체계가 필요하다. 이 세 문장을 입력·변환·출력의 순서로 다시 써 보고, 유사도 계산, 투영, 선형 계층, 어텐션 점수와 에너지 계산에 사용한다.라는 적용 범위에서 벡터 크기에 영향을 받으므로 방향 유사도만 비교할 때는 정규화가 필요하고 복소수에서는 켤레 규칙을 확인해야 한다.라는 한계가 어떤 관측 지표로 나타나는지 정의한다. 두 문서 임베딩의 내적과 코사인 유사도를 계산해 벡터 정규화 전후 순위가 어떻게 달라지는지 확인한다. 예시를 정상·경계·실패 사례로 나누고 각 사례의 기대 결과와 허용 오차, 중단 조건을 표로 만든다. 마지막으로 관련 문서와의 차이를 용어가 아니라 데이터 요구량, 계산 절차, 출력 형식과 실패 비용으로 비교한다.
...
<span id="reference-4"></span>4. [Dot product — Wikipedia](https://en.wikipedia.org/wiki/Dot_product) — encyclopedia
