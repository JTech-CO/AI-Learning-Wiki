---
title: "감지-사고-행동 순환 Sense-Think-Act Cycle"
description: "센서 입력을 수집하고 해석한 뒤 행동을 선택해 환경에 적용하는 세 단계 제어 모형이다."
tableOfContents: { minHeadingLevel: 2, maxHeadingLevel: 4 }
---

<p class="wiki-lead">센서 입력을 수집하고 해석한 뒤 행동을 선택해 환경에 적용하는 세 단계 제어 모형이다.</p>
...
센서 입력을 수집하고 해석한 뒤 행동을 선택해 환경에 적용하는 세 단계 제어 모형이다.

감지 단계가 신호를 상태로 바꾸고 사고 단계가 규칙이나 모델로 행동을 고르며 행동 단계가 명령을 실행한다. 이 표제어는 Sense-Think-Act Cycle이라는 영문 용어와 함께 사용되며, 구현 이름보다 입력·처리·출력과 성립 조건을 기준으로 이해해야 한다. 핵심 정의를 실제 시스템에 적용할 때는 모델이나 도구의 버전, 데이터 범위와 평가 단위를 함께 적어 같은 이름 아래 다른 동작이 섞이지 않게 한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a></div>

### 배경과 설명 범위

센서 입력을 수집하고 해석한 뒤 행동을 선택해 환경에 적용하는 세 단계 제어 모형이다. 설명 범위에는 개념의 역사적 배경뿐 아니라 현재의 계산 절차와 운영 경계가 포함된다. 인식-계획-행동 순환과 유사하지만 계획을 별도 산출물로 요구하지 않으며 반응형 제어까지 포함하는 더 넓은 표현이다.
...
감지 단계가 신호를 상태로 바꾸고 사고 단계가 규칙이나 모델로 행동을 고르며 행동 단계가 명령을 실행한다.
...
센서, 지각 처리, 세계 모형, 의사결정기, 구동기와 피드백 채널이 핵심 요소다.
...
모바일 로봇, 자율 시스템과 실시간 모니터링에서 제어 경계를 설명하는 데 쓰인다.
...
세 단계를 엄격히 직렬화하면 빠르게 변하는 환경에서 지연이 커지고, 감지 오류를 사고 오류로 오해할 수 있다.

감지-사고-행동 순환을 에이전트 시스템에서 해석할 때는 목표, 관찰 가능한 상태, 사용할 수 있는 도구, 행동 결과와 종료 조건을 분리해야 한다. 같은 결과를 내더라도 계획이 갱신되는 시점과 실패를 복구하는 방식이 다르면 서로 다른 설계다. 실행 기록에는 선택한 행동뿐 아니라 제외한 대안, 도구 응답, 재시도 횟수와 사람에게 이관한 조건을 남겨야 한다. 평균값만으로 한계를 숨기지 않도록 드문 입력, 분포 변화, 악의적 조작과 자원 고갈 조건을 따로 시험한다. 알려진 실패를 탐지할 지표와 자동 중단선, 안전한 대체 경로, 사람이 검토할 사건을 사전에 정한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

### 관련 개념과의 구분

인식-계획-행동 순환과 유사하지만 계획을 별도 산출물로 요구하지 않으며 반응형 제어까지 포함하는 더 넓은 표현이다.
...
창고 로봇이 장애물을 감지하고 우회 방향을 계산해 속도를 조정한 뒤 새 센서 입력으로 안전거리를 다시 확인한다.
...
1. **목적과 경계 정의:** 감지-사고-행동 순환이 해결할 문제와 해결하지 않을 문제를 각각 한 문장으로 적는다.
...
센서 입력을 수집하고 해석한 뒤 행동을 선택해 환경에 적용하는 세 단계 제어 모형이다. 감지 단계가 신호를 상태로 바꾸고 사고 단계가 규칙이나 모델로 행동을 고르며 행동 단계가 명령을 실행한다. 센서, 지각 처리, 세계 모형, 의사결정기, 구동기와 피드백 채널이 핵심 요소다. 이 세 문장을 입력·변환·출력의 순서로 다시 써 보고, 모바일 로봇, 자율 시스템과 실시간 모니터링에서 제어 경계를 설명하는 데 쓰인다.라는 적용 범위에서 세 단계를 엄격히 직렬화하면 빠르게 변하는 환경에서 지연이 커지고, 감지 오류를 사고 오류로 오해할 수 있다.라는 한계가 어떤 관측 지표로 나타나는지 정의한다. 창고 로봇이 장애물을 감지하고 우회 방향을 계산해 속도를 조정한 뒤 새 센서 입력으로 안전거리를 다시 확인한다. 예시를 정상·경계·실패 사례로 나누고 각 사례의 기대 결과와 허용 오차, 중단 조건을 표로 만든다. 마지막으로 관련 문서와의 차이를 용어가 아니라 데이터 요구량, 계산 절차, 출력 형식과 실패 비용으로 비교한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

### 학습 체크

- 감지-사고-행동 순환의 입력, 처리와 출력을 각각 설명할 수 있는가?
