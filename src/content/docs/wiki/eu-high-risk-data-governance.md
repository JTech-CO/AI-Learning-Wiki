---
title: "EU 고위험 AI 데이터 거버넌스 EU High-Risk AI Data Governance"
description: "EU 고위험 AI 데이터 거버넌스는 학습·검증·시험 데이터의 목적 적합성, 품질, 대표성과 편향 통제를 수명주기 절차로 관리하는 요구사항이다."
tableOfContents: { minHeadingLevel: 2, maxHeadingLevel: 4 }
---

<p class="wiki-alias">AI Act Data Governance · AI Act Article 10</p>

<p class="wiki-lead">EU 고위험 AI 데이터 거버넌스는 학습·검증·시험 데이터의 목적 적합성, 품질, 대표성과 편향 통제를 수명주기 절차로 관리하는 요구사항이다.</p>
...
EU 고위험 AI 데이터 거버넌스는 학습·검증·시험 데이터의 목적 적합성, 품질, 대표성과 편향 통제를 수명주기 절차로 관리하는 요구사항이다.

EU AI Act Article 10을 중심으로 고위험 시스템 개발에 사용되는 데이터와 데이터 기반이 아닌 시스템의 관련 처리 요구를 구분한다. 개인정보 처리는 GDPR 등 별도 법률의 적법 근거와 원칙을 함께 충족해야 한다.

‘EU 고위험 AI 데이터 거버넌스(EU High-Risk AI Data Governance)’를 검토할 때는 먼저 관할, 적용 시점, 규율 대상과 조직의 역할을 고정한다. 법률 본문, 시행령·위임 규정, 감독기관 지침은 법적 효력과 갱신 주기가 다르므로 한 문장에 섞지 않는다. 이 문서는 2026년 8월 30일 현재의 공식 자료를 바탕으로 한 학습 정보이며 개별 사건의 법률 의견을 대신하지 않는다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

### 적용 범위와 판단 경계

거버넌스는 설계 선택, 수집·출처·원천, 준비·정제·라벨링, 가정, 가용성·수량·적합성, 편향 조사, 격차·결함과 사용 맥락의 대표성을 다룬다. ‘완벽하게 편향 없는 데이터’를 요구하는 것이 아니라 식별 가능한 위험을 체계적으로 통제한다.
...
의도된 목적과 대상 모집단에서 필요한 데이터 특성을 정의하고 데이터셋별 계보·권리·품질·대표성·오염·누락을 평가한다. 문제를 수정·제한·가중하거나 사용 범위를 좁히고 모델 시험과 사후시장 신호로 가정을 갱신한다.
...
데이터시트, 수집·라이선스·처리 이력, 분할 기준, 품질·대표성·편향 지표, 제외·수정 결정, 승인자와 모델 버전 연결을 보존한다. 원데이터에 접근하지 못하는 외부 모델은 공급자 문서와 검증 한계를 명시한다.
...
데이터 품질 점수 하나나 다양성 비율만으로 Article 10 준수가 증명되지 않는다. 대표성은 의도된 목적과 지역·시간에 의존하고, 특수범주 개인정보를 편향 탐지에 쓰는 경우에도 법정 조건과 보호조치가 필요하다.
...
모델 버전에서 사용 데이터와 처리 코드를 역추적하고 주요 집단·경계 사례의 성능과 오류를 재현한다. 실제 사용 분포가 바뀌거나 새로운 피해가 관찰되면 데이터 가정과 통제를 갱신한다.
...
- [학습 데이터 출처 추적](/wiki/training-data-provenance/): 법적 범위와 통제를 이해하기 위한 선수 개념이다.

#### 함께 비교할 문서

- [편향과 공정성](/wiki/bias-fairness/): 관할·역할·수명주기 단계가 다른 인접 개념이다.
- [개인정보 보호](/wiki/privacy/): 관할·역할·수명주기 단계가 다른 인접 개념이다.
- [학습 감사 로그](/wiki/training-audit-log/): 관할·역할·수명주기 단계가 다른 인접 개념이다.
...
- EU 고위험 AI 데이터 거버넌스의 관할, 적용 시점, 대상 주체와 대상 시스템을 구분해 설명할 수 있는가?
...
- **AI 규제와 리터러시:** [다음 문서 — 편향과 공정성](/wiki/bias-fairness/)
