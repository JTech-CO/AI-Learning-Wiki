...
이 사례에 ‘오픈 웨이트 모델(Open-Weight Model)’을 적용한다면 먼저 성공 조건과 금지 조건을 적고 기준선 결과를 저장한다. 그다음 가중치 공개는 학습 코드, 데이터와 자유로운 사용 권한까지 자동으로 보장하지 않으므로 라이선스 범위를 따로 확인해야 한다. 입력과 중간 상태, 최종 결과를 단계별로 수집하고 정상 사례, 경계 사례, 의도적인 실패 사례를 같은 절차로 실행한다.
