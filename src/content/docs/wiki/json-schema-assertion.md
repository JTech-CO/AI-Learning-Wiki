---
title: "JSON 스키마 단언 JSON Schema Assertion"
description: "JSON 스키마 단언은 인스턴스가 자료형, 범위, 패턴과 필수 속성 같은 제약을 만족하는지 참 또는 거짓으로 판정하는 키워드 평가다."
tableOfContents: { minHeadingLevel: 2, maxHeadingLevel: 4 }
---

<p class="wiki-alias">검증 단언</p>

<p class="wiki-lead">JSON 스키마 단언은 인스턴스가 자료형, 범위, 패턴과 필수 속성 같은 제약을 만족하는지 참 또는 거짓으로 판정하는 키워드 평가다.</p>
...
JSON 스키마 단언은 인스턴스가 자료형, 범위, 패턴과 필수 속성 같은 제약을 만족하는지 참 또는 거짓으로 판정하는 키워드 평가다.

type, required, minimum, pattern과 enum은 대표적인 단언 키워드다. 단언 결과는 인스턴스 위치와 스키마 위치에 연결해 어떤 값이 어떤 제약을 위반했는지 보고해야 한다.

‘JSON 스키마 단언(JSON Schema Assertion)’를 이해할 때는 명칭만 외우지 않고 무엇을 입력으로 받고, 어떤 상태나 규칙을 적용하며, 어떤 결과를 관찰하는지 나누어 본다. 이 구분은 비슷한 기능을 제공하는 모델·도구·표준을 비교할 때 같은 단어가 서로 다른 의미로 쓰이는 문제를 줄인다. 또한 결과가 유효하다고 판단할 기준과 판단을 보류해야 할 조건을 함께 적어야 실제 학습과 운영에 연결할 수 있다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

### 배경과 설명 범위

단언은 유효성에 영향을 주고 주석은 설명 정보를 제공한다. 같은 스키마에 둘 다 있을 수 있지만 오류 처리에서는 반드시 구분한다.

이 문서의 범위는 ‘JSON 스키마 단언’의 안정적인 정의, 핵심 처리 흐름, 적용 조건과 실패 경계다. 특정 제품의 가격·기본값·성능 수치처럼 빠르게 바뀌는 정보는 일반 원리와 분리하고, 구현을 선택할 때는 연결된 공식 문서와 배포 환경의 버전을 다시 확인한다.
...
검증기는 현재 인스턴스에 적용되는 키워드 조건을 계산하고 실패를 누적하거나 빠르게 중단한다. allOf, anyOf와 조건부 스키마는 여러 하위 단언의 논리 결과를 결합한다.
...
‘JSON 스키마 단언(JSON Schema Assertion)’의 구현을 비교할 때는 정상 사례 하나만 보지 않는다. 경계값, 빈 입력, 큰 입력, 일부 정보가 누락된 입력과 의도적인 실패 사례를 같은 절차로 실행하고, 값이 달라진 이유가 데이터·설정·알고리즘·운영 자원 중 어디에 있는지 분리한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

### 구성 요소와 데이터 흐름

‘JSON 스키마 단언’를 시스템에 넣을 때는 사용자 또는 호출자의 입력 인터페이스, 핵심 상태와 계산부, 정책·설정, 결과 검증부, 관측과 오류 처리부로 나눈다. 구성 요소 사이에는 자료형, 식별자, 단위, 시간 제한과 오류 전달 규칙을 명시한다.

type, required, minimum, pattern과 enum은 대표적인 단언 키워드다. 단언 결과는 인스턴스 위치와 스키마 위치에 연결해 어떤 값이 어떤 제약을 위반했는지 보고해야 한다. 이 원리를 데이터 흐름으로 표현하면 어떤 값이 영구 상태인지, 요청 동안만 유지되는지, 외부 근거에서 오는지 구분할 수 있다. 내부 구현을 바꾸더라도 입력·출력 계약과 검증 사례를 유지하면 교체 전후의 동작을 재현 가능한 방식으로 비교할 수 있다.

문서·코스·도구 연결에서는 ‘JSON 스키마 단언’ 문서를 ‘prompt-systems’ 코스와 ‘prompt-schema’ 도구가 공유한다. 코스는 읽기 순서를 제공하고 도구는 입력값으로 원리를 확인하게 하므로, 용어 정의와 실습 결과가 다른 기준을 사용하지 않도록 같은 선수·관련 문서 ID를 사용한다.
...
모델의 구조화 출력, API 요청·응답, 설정 파일과 이벤트 메시지의 계약 검증에 사용한다. 배포 전 예시와 실패 사례를 함께 고정하면 스키마 변경의 영향을 찾을 수 있다.

도입 여부는 유행이나 제품 이름이 아니라 해결하려는 문제와 측정 가능한 개선으로 결정한다. 먼저 현재 방식의 품질, 오류, 지연 시간, 자원, 비용과 사람 개입을 기준선으로 기록한다. 그다음 ‘JSON 스키마 단언’를 적용한 같은 사례에서 개선된 항목과 악화된 항목을 함께 비교한다.
...
스키마가 통과해도 데이터가 사실이거나 업무적으로 안전하다는 뜻은 아니다. format처럼 방언과 설정에 따라 주석 또는 단언으로 처리되는 키워드도 있어 검증기 옵션을 기록해야 한다.
...
- [응답 검증](/wiki/response-validation/): 입력·출력·목적 또는 실패 조건을 나란히 비교할 관련 개념이다.
...
분류 결과에 type object, required label, enum 세 값과 additionalProperties false를 단언해 설명문이 섞이거나 허용되지 않은 라벨이 오는 경우를 차단한다.

이 사례를 검증할 때는 적용 전 입력과 기준선 결과를 보존하고, ‘JSON 스키마 단언’를 적용한 뒤 바뀐 설정과 중간 상태를 순서대로 기록한다. 결과 표에는 성공 여부만 두지 않고 품질, 비용, 지연, 자원, 보류·사람 개입 횟수와 남은 불확실성을 포함한다.
...
1. **목표 정의:** ‘JSON 스키마 단언’로 바꾸려는 결과와 바꾸지 않을 범위를 각각 한 문장으로 적는다.
...
- JSON 스키마 단언의 입력, 처리 규칙과 출력을 서로 구분해 설명할 수 있는가?
- JSON 스키마 어휘와 어떤 선후 관계가 있는지 사례로 설명할 수 있는가?
- 다음 한계를 실제 사례에서 찾을 수 있는가? 스키마가 통과해도 데이터가 사실이거나 업무적으로 안전하다는 뜻은 아니다. format처럼 방언과 설정에 따라 주석 또는 단언으로 처리되는 키워드도 있어 검증기 옵션을 기록해야 한다.
...
- **프롬프트 시스템 설계:** [다음 문서 — 동적 스키마 참조](/wiki/dynamic-reference/)
