...
<p class="wiki-lead">실제 관측값을 그대로 복제하지 않고 규칙·시뮬레이션·통계 또는 생성 모델로 만든 데이터다.</p>
...
[학습 데이터](/wiki/training-data/) 및 [데이터 증강](/wiki/data-augmentation/)를 먼저 보면 입력과 출력의 위치를 구분하기 쉽다.
...
- [개인정보 보호](/wiki/privacy/): 개인정보 보호는 합성 방식의 가능한 목표 중 하나지만 합성만으로 자동 달성되지는 않는다.
...
4. **실패 조건 시험:** 분포·상관관계·희귀 집단의 통계적 충실도, 실제 시험셋에서의 과제 효용, 중복과 개인정보 공격 위험을 분리해 측정한다.
...
- **관련 검토:** [privacy](/wiki/privacy/), [dataset](/wiki/dataset/), [evaluation](/wiki/evaluation/) 문서의 역할과 경계를 함께 확인해 인접 단계의 오류를 합성 데이터의 문제로 잘못 진단하지 않는다.
...
- **데이터·학습 파이프라인:** [다음 문서 — 데이터 증강](/wiki/data-augmentation/)
