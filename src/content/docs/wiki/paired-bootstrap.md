---
title: "대응 부트스트랩 Paired Bootstrap"
description: "대응 부트스트랩은 같은 평가 사례에 대한 두 시스템의 결과 쌍을 함께 재표집해 지표 차이의 불확실성을 추정하는 방법이다."
tableOfContents: { minHeadingLevel: 2, maxHeadingLevel: 4 }
---

<p class="wiki-alias">Paired Bootstrap Resampling · 대응 표본 부트스트랩</p>

<p class="wiki-lead">대응 부트스트랩은 같은 평가 사례에 대한 두 시스템의 결과 쌍을 함께 재표집해 지표 차이의 불확실성을 추정하는 방법이다.</p>
...
대응 부트스트랩은 같은 평가 사례에 대한 두 시스템의 결과 쌍을 함께 재표집해 지표 차이의 불확실성을 추정하는 방법이다.

두 모델을 서로 다른 표본으로 재표집하지 않고 동일한 사례 인덱스를 뽑아 대응 관계를 보존한다. 관심값은 각 재표집에서 계산한 두 시스템 점수의 차이다.

‘대응 부트스트랩(Paired Bootstrap)’를 이해할 때는 명칭만 외우지 않고 무엇을 입력으로 받고, 어떤 상태나 규칙을 적용하며, 어떤 결과를 관찰하는지 나누어 본다. 이 구분은 비슷한 기능을 제공하는 모델·도구·표준을 비교할 때 같은 단어가 서로 다른 의미로 쓰이는 문제를 줄인다. 또한 결과가 유효하다고 판단할 기준과 판단을 보류해야 할 조건을 함께 적어야 실제 학습과 운영에 연결할 수 있다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

### 배경과 설명 범위

일반 부트스트랩이 한 통계량의 분포를 추정한다면 대응 부트스트랩은 같은 사례에서 나온 두 결과의 상관을 보존한 차이 통계량에 초점을 둔다.

이 문서의 범위는 ‘대응 부트스트랩’의 안정적인 정의, 핵심 처리 흐름, 적용 조건과 실패 경계다. 특정 제품의 가격·기본값·성능 수치처럼 빠르게 바뀌는 정보는 일반 원리와 분리하고, 구현을 선택할 때는 연결된 공식 문서와 배포 환경의 버전을 다시 확인한다.
...
크기 n인 평가 집합에서 복원추출로 n개 인덱스를 뽑고 두 시스템의 해당 결과를 함께 선택한다. 이 절차를 반복해 점수 차이 분포, 신뢰구간과 방향 일관성을 계산한다.
...
‘대응 부트스트랩(Paired Bootstrap)’의 구현을 비교할 때는 정상 사례 하나만 보지 않는다. 경계값, 빈 입력, 큰 입력, 일부 정보가 누락된 입력과 의도적인 실패 사례를 같은 절차로 실행하고, 값이 달라진 이유가 데이터·설정·알고리즘·운영 자원 중 어디에 있는지 분리한다.

<div class="wiki-section-sources" aria-label="이 구획의 근거"><span>근거</span> <a href="#reference-1">[1]</a> <a href="#reference-2">[2]</a></div>

### 구성 요소와 데이터 흐름

‘대응 부트스트랩’를 시스템에 넣을 때는 사용자 또는 호출자의 입력 인터페이스, 핵심 상태와 계산부, 정책·설정, 결과 검증부, 관측과 오류 처리부로 나눈다. 구성 요소 사이에는 자료형, 식별자, 단위, 시간 제한과 오류 전달 규칙을 명시한다.

두 모델을 서로 다른 표본으로 재표집하지 않고 동일한 사례 인덱스를 뽑아 대응 관계를 보존한다. 관심값은 각 재표집에서 계산한 두 시스템 점수의 차이다. 이 원리를 데이터 흐름으로 표현하면 어떤 값이 영구 상태인지, 요청 동안만 유지되는지, 외부 근거에서 오는지 구분할 수 있다. 내부 구현을 바꾸더라도 입력·출력 계약과 검증 사례를 유지하면 교체 전후의 동작을 재현 가능한 방식으로 비교할 수 있다.

문서·코스·도구 연결에서는 ‘대응 부트스트랩’ 문서를 ‘llm-evaluation’ 코스와 ‘evaluation-metrics’ 도구가 공유한다. 코스는 읽기 순서를 제공하고 도구는 입력값으로 원리를 확인하게 하므로, 용어 정의와 실습 결과가 다른 기준을 사용하지 않도록 같은 선수·관련 문서 ID를 사용한다.
...
번역, 요약, 질의응답과 분류 모델의 동일 테스트셋 비교에 사용한다. 비선형 지표처럼 단순 표준오차 계산이 어려운 경우에도 재표집 단위를 명확히 하면 적용할 수 있다.

도입 여부는 유행이나 제품 이름이 아니라 해결하려는 문제와 측정 가능한 개선으로 결정한다. 먼저 현재 방식의 품질, 오류, 지연 시간, 자원, 비용과 사람 개입을 기준선으로 기록한다. 그다음 ‘대응 부트스트랩’를 적용한 같은 사례에서 개선된 항목과 악화된 항목을 함께 비교한다.
...
평가 사례가 독립적이지 않거나 사용자·문서 묶음 구조가 있으면 행 단위 재표집이 불확실성을 과소평가한다. 이때는 군집 단위를 함께 뽑고 반복 횟수와 난수 시드를 기록한다.
...
- [부트스트랩 방법](/wiki/bootstrap-method/): 이 문서를 이해하기 위한 선수 개념이다.
- [모델 평가](/wiki/evaluation/): 이 문서를 이해하기 위한 선수 개념이다.

#### 함께 비교할 문서

- [부트스트랩 신뢰구간](/wiki/bootstrap-confidence-interval/): 입력·출력·목적 또는 실패 조건을 나란히 비교할 관련 개념이다.
...
같은 500개 질의에 대한 모델 A와 B의 F1 차이를 비교할 때 질의 인덱스를 5,000번 재표집하고 매번 A-B를 계산해 95퍼센트 구간이 0을 포함하는지 확인한다.

이 사례를 검증할 때는 적용 전 입력과 기준선 결과를 보존하고, ‘대응 부트스트랩’를 적용한 뒤 바뀐 설정과 중간 상태를 순서대로 기록한다. 결과 표에는 성공 여부만 두지 않고 품질, 비용, 지연, 자원, 보류·사람 개입 횟수와 남은 불확실성을 포함한다.
...
1. **목표 정의:** ‘대응 부트스트랩’로 바꾸려는 결과와 바꾸지 않을 범위를 각각 한 문장으로 적는다.
2. **선수 확인:** [부트스트랩 방법](/wiki/bootstrap-method/), [모델 평가](/wiki/evaluation/)의 정의와 입력 조건을 먼저 확인한다.
...
- 대응 부트스트랩의 입력, 처리 규칙과 출력을 서로 구분해 설명할 수 있는가?
- 부트스트랩 방법·모델 평가와 어떤 선후 관계가 있는지 사례로 설명할 수 있는가?
- 다음 한계를 실제 사례에서 찾을 수 있는가? 평가 사례가 독립적이지 않거나 사용자·문서 묶음 구조가 있으면 행 단위 재표집이 불확실성을 과소평가한다. 이때는 군집 단위를 함께 뽑고 반복 횟수와 난수 시드를 기록한다.
...
- **LLM 평가와 관측성:** [다음 문서 — 최소 검출 가능 효과](/wiki/minimum-detectable-effect/)
