...
if (queueById.size !== 1400 || topicById.size !== 1400) errors.push('W10 queue must contain 1,400 unique topics');
if (readyIds.size !== 350) errors.push(`expected 350 unique ready articles, found ${readyIds.size}`);
for (const [topicId, topic] of topicById) {
  ...
  if (topic.state === 'existing' && !item.publicationReady) errors.push(`${topicId}: existing article is not publication-ready after W10`);
}
...
for (const category of categories.categories) {
  ...
  if (!categorySummary || categorySummary.topics !== 100 || categorySummary.published !== 25 || categorySummary.existingQueued !== 0 || categorySummary.candidates !== 75) errors.push(`${category.id}: expected 25 published and 75 candidates`);
}
...
if (queue.totals.existing !== 350 || queue.totals.candidates !== 1050 || queue.totals.published !== 350 || queue.totals.sourceRemediation !== 0 || queue.totals.claimReviewQueued !== 0 || queue.totals.researchQueued !== 1050) errors.push('W10 totals must be 350 published existing and 1,050 queued candidates');
if (errors.length) {
  console.error(`W10 production queue validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W10 production queue validation: 1,400 topics; 350 published, 1,050 candidates, 25 per category');
