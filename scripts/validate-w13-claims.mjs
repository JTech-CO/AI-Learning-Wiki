...
import { familyForW13 } from './w13-content-lib.mjs';
import { topicFacts } from './w13-topic-facts.mjs';

const readJson = async (file) => JSON.parse(await readFile(file, 'utf8'));
const sha = (value) => createHash('sha256').update(value).digest('hex');
const segmenter = new Intl.Segmenter('ko', { granularity: 'sentence' });
const claimsOf = (body) => [...segmenter.segment(body)].map(({ segment }) => segment.trim()).filter(Boolean);
const priorMilestones = ['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12'];
const [schema, ledger, rules, summary, manifest, w2, verification, categories, ...priorLedgers] = await Promise.all([
  ...
  readJson('content-model/evidence/w13-review-rules.json'),
  readJson('content-model/evidence/w13-summary.json'),
  readJson('content-model/evidence/w13-batch-manifest.json'),
...
const rulesText = await readFile('content-model/evidence/w13-review-rules.json', 'utf8');
if (ledger.rulesSha256 !== sha(rulesText)) errors.push('rulesSha256 differs from W13 review rules');
...
for (const reviewed of ledger.articles) {
  ...
  if (!rule || !manifestItem) errors.push(`${article.id}: W13 rule or manifest item missing`);
  ...
  if (rule?.priorStage !== 'research-queued' || manifestItem?.previousStage !== 'research-queued' || manifestItem?.previousState !== 'candidate') errors.push(`${article.id}: did not enter W13 from the candidate research gate`);
  if (!w2ById.get(article.id)?.audit.readyForManualClaimReview) errors.push(`${article.id}: W2 evidence gate not met`);
  if (article.reviewedAt !== rules.reviewedAt || article.status !== 'reviewed') errors.push(`${article.id}: review metadata differs from W13 rules`);
  ...
  const families = new Set(article.sources.map(familyForW13));
...
if (articleCount < 770) errors.push(`expected at least 770 article files after W13, found ${articleCount}`);
if (ledger.articles.length !== 140 || rules.articles.length !== 140 || manifest.topics.length !== 140) errors.push('W13 must contain exactly 140 new articles');
if (reviewedIds.size !== 140) errors.push('W13 reviewed article IDs are not unique');
for (const category of categories.categories) if (reviewedCategories.get(category.id) !== 10) errors.push(`${category.id}: expected 10 W13 articles`);
if (manifest.totals.newlyCreated !== 140 || manifest.totals.cumulativeArticles !== 770 || manifest.slices.length !== 10 || manifest.slices.some((slice) => slice.topics.length !== 14)) errors.push('W13 batch manifest totals are invalid');
if (summary.batchReviewedArticles !== 140 || summary.cumulativeReviewedArticles !== 770 || summary.publicationReadyArticles !== 140 || summary.newlyCreatedArticles !== 140 || summary.existingArticles !== 770 || summary.candidateTopics !== 630 || summary.topicsPerCategory !== 55 || !summary.bodyHashLocked) errors.push('W13 summary is incomplete');
if (summary.reviewedClaimUnits !== claimCount || summary.acceptedClaimUnits !== claimCount) errors.push('W13 summary claim counts differ from ledger');
if (verification.totals.unavailable !== 0 || verification.totals.restricted !== 0) errors.push('W13 source verification contains unavailable or restricted URLs');

if (errors.length) {
  console.error(`W13 claim validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log(`W13 claim validation: 140 new articles, ${claimCount} locked claims, 770 publication-ready cumulatively`);
