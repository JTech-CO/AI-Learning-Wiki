...
const ledgers = await Promise.all(['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11'].map(async (milestone) => JSON.parse(await readFile(`content-model/evidence/${milestone}-claim-ledger.json`, 'utf8'))));
...
for (const articleId of articleDirs) {
  ...
  if (w11.articles.some((article) => article.articleId === articleId)) {
...
if (readyPages !== 490) errors.push(`expected 490 rendered ready pages, found ${readyPages}`);
if (errors.length) {
  console.error(`W11 rendered status check: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W11 rendered status check: exactly 490 publication-ready pages are labeled and W11 evidence anchors resolve');
