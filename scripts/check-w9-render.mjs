...
const ledgers = await Promise.all(['w4', 'w5', 'w6', 'w7', 'w8', 'w9'].map(async (milestone) => JSON.parse(await readFile(`content-model/evidence/${milestone}-claim-ledger.json`, 'utf8'))));
...
if (errors.length) {
  console.error(`W9 rendered status check: ${errors.length} error(s)\n${errors.slice(0, 160).join('\n')}`);
  process.exit(1);
}
console.log('W9 rendered status check: exactly 210 publication-ready pages are labeled');
