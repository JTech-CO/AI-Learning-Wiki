...
const targetQueueItems = selectW14Targets(queue);
...
for (const queueItem of targetQueueItems) {
  ...
  if (queueItem.state !== 'candidate' || queueItem.stage !== 'research-queued') errors.push(`${topic.id}: W14 target is not a queued candidate`);
  if (!topicFacts[topic.id]) errors.push(`${topic.id}: curated W14 fact profile missing`);
  const file = path.join('content-model', 'articles', `${topic.id}.article.json`);
  const sources = sourcesForW14(topic, research, wikimedia, registry);
  const families = new Set(sources.map(familyForW14));
  ...
  const article = createW14Article(topic, sources);
...
for (const categoryId of categoryIds) {
  ...
  if (members.length !== 10) errors.push(`${categoryId}: expected 10 W14 topics, found ${members.length}`);
}
if (categoryIds.length !== 14) errors.push(`expected 14 categories, found ${categoryIds.length}`);
if (targetQueueItems.length !== 140 || manifestTopics.length !== 140) errors.push(`expected 140 new W14 topics, found ${manifestTopics.length}`);
if (new Set(targetQueueItems.map((topic) => topic.topicId)).size !== 140) errors.push('W14 target IDs are not unique');
if (errors.length) throw new Error(`W14 preparation failed:\n${errors.join('\n')}`);

const slices = Array.from({ length: 10 }, (_, index) => ({
  id: `w14-slice-${String(index + 1).padStart(2, '0')}`,
...
const manifest = {
  ...
  totals: { topics: 140, newlyCreated: 140, cumulativeArticles: 910 },
  slices,
  topics: manifestTopics,
};
const rules = { version: VERSION, reviewedAt: REVIEWED_AT, reviewer: 'Codex W14 expansion review', policy, factualSectionIds: FACTUAL_SECTION_IDS, articles: reviewRules };

await mkdir('content-model/evidence', { recursive: true });
await Promise.all([
  writeFile('content-model/evidence/w14-batch-manifest.json', `${JSON.stringify(manifest, null, 2)}\n`, 'utf8'),
  writeFile('content-model/evidence/w14-review-rules.json', `${JSON.stringify(rules, null, 2)}\n`, 'utf8'),
]);
console.log(`W14 preparation: ${manifestTopics.length} new articles in ${slices.length} resumable slices; 910 cumulative`);
