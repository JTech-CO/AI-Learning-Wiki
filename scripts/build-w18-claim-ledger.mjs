...
const priorLedgers = await Promise.all(['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14', 'w15', 'w16', 'w17'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)));
...
const summary = {
  ...
  topicsPerCategory: 100,
  nextGate: 'W19에서 1,400개 문서의 깊이·최신성·상호 연결 품질을 유지보수',
};

await Promise.all([
  writeFile('content-model/evidence/w18-claim-ledger.json', `${JSON.stringify(ledger, null, 2)}\n`, 'utf8'),
  writeFile('content-model/evidence/w18-summary.json', `${JSON.stringify(summary, null, 2)}\n`, 'utf8'),
]);
console.log(`W18 claim ledger: ${articles.length} articles, ${summary.reviewedSections} sections, ${claims.length} sentence claims locked; ${summary.cumulativeReviewedArticles} cumulative`);
