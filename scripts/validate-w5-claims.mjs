...
const [schema, ledger, rules, summary, w2, w4, categories] = await Promise.all([
  readJson('content-model/schema.claim-ledger.json'),
  readJson('content-model/evidence/w5-claim-ledger.json'),
  readJson('content-model/evidence/w5-review-rules.json'),
  readJson('content-model/evidence/w5-summary.json'),
...
const rulesText = await readFile('content-model/evidence/w5-review-rules.json', 'utf8');
...
const w4Ids = new Set(w4.articles.filter((item) => item.publicationReady).map((item) => item.articleId));
...
for (const reviewed of ledger.articles) {
  ...
  if (w4Ids.has(article.id)) errors.push(`${article.id}: W5 batch overlaps W4`);
  if (!rule || !article.categories.includes(rule.categoryId)) errors.push(`${article.id}: category assignment differs from W5 rules`);
...
if (ledger.articles.length !== 14 || rules.articles.length !== 14) errors.push(`expected 14 W5 articles, found ${ledger.articles.length}`);
if (reviewedCategories.size !== 14 || [...expectedCategories].some((id) => !reviewedCategories.has(id))) errors.push('W5 batch must contain exactly one article from every category');
if (summary.batchReviewedArticles !== 14 || summary.cumulativeReviewedArticles !== 28 || summary.reviewedSections !== 126 || summary.publicationReadyArticles !== 14 || !summary.bodyHashLocked) errors.push('W5 publication gate summary is incomplete');
if (summary.reviewedClaimUnits !== claimCount || summary.acceptedClaimUnits !== claimCount) errors.push('W5 summary counts differ from the locked ledger');

if (errors.length) {
  console.error(`W5 claim validation: ${errors.length} error(s)\n${errors.slice(0, 80).join('\n')}`);
  process.exit(1);
}
console.log(`W5 claim validation: 14 category-expansion articles, 126 sections, ${claimCount} locked sentence claims; 28 publication-ready cumulatively`);
