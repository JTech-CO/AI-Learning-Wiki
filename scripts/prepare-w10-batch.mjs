...
const targetQueueItems = selectW10Targets(queue);
...
for (const queueItem of targetQueueItems) {
  ...
  if (queueItem.state !== 'candidate' || queueItem.stage !== 'research-queued') errors.push(`${topic.id}: W10 target is not a queued candidate`);
  if (!topicFacts[topic.id]) errors.push(`${topic.id}: curated W10 fact profile missing`);
  const file = path.join('content-model', 'articles', `${topic.id}.article.json`);
  const sources = sourcesForW10(topic, research, wikimedia, registry);
  const families = new Set(sources.map(familyForW10));
  ...
  const article = createW10Article(topic, sources);
...
for (const categoryId of categoryIds) {
  ...
  if (members.length !== 10) errors.push(`${categoryId}: expected 10 W10 topics, found ${members.length}`);
}
if (categoryIds.length !== 14) errors.push(`expected 14 categories, found ${categoryIds.length}`);
if (targetQueueItems.length !== 140 || manifestTopics.length !== 140) errors.push(`expected 140 new W10 topics, found ${manifestTopics.length}`);
if (new Set(targetQueueItems.map((topic) => topic.topicId)).size !== 140) errors.push('W10 target IDs are not unique');
if (errors.length) throw new Error(`W10 preparation failed:\n${errors.join('\n')}`);

const slices = Array.from({ length: 10 }, (_, index) => ({
  id: `w10-slice-${String(index + 1).padStart(2, '0')}`,
...
const manifest = {
  ...
  totals: { topics: 140, newlyCreated: 140, cumulativeArticles: 350 },
  slices,
  topics: manifestTopics,
};
const rules = { version: VERSION, reviewedAt: REVIEWED_AT, reviewer: 'Codex W10 expansion review', policy, factualSectionIds: FACTUAL_SECTION_IDS, articles: reviewRules };

await mkdir('content-model/evidence', { recursive: true });
await Promise.all([
  writeFile('content-model/evidence/w10-batch-manifest.json', `${JSON.stringify(manifest, null, 2)}\n`, 'utf8'),
  writeFile('content-model/evidence/w10-review-rules.json', `${JSON.stringify(rules, null, 2)}\n`, 'utf8'),
]);
console.log(`W10 preparation: ${manifestTopics.length} new articles in ${slices.length} resumable slices; 350 cumulative`);
