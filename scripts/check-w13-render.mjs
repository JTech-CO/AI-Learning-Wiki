...
const ledgers = await Promise.all(['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13'].map(async (milestone) => JSON.parse(await readFile(`content-model/evidence/${milestone}-claim-ledger.json`, 'utf8'))));
...
if (readyPages !== 770) errors.push(`expected 770 rendered ready pages, found ${readyPages}`);
if (errors.length) {
  console.error(`W13 rendered status check: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W13 rendered status check: exactly 770 publication-ready pages are labeled and W13 evidence anchors resolve');
