...
const [schema, queue, topics, batches, evidence, categories, ...ledgers] = await Promise.all([
  ...
  ...['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)),
]);
const errors = [];
...
if (queueById.size !== 1400 || topicById.size !== 1400) errors.push('W14 queue must contain 1,400 unique topics');
if (readyIds.size !== 910) errors.push(`expected 910 unique ready articles, found ${readyIds.size}`);
for (const [topicId, topic] of topicById) {
  ...
  if (topic.state === 'existing' && !item.publicationReady) errors.push(`${topicId}: existing article is not publication-ready after W14`);
}
...
for (const category of categories.categories) {
  ...
  if (!categorySummary || categorySummary.topics !== 100 || categorySummary.published !== 65 || categorySummary.existingQueued !== 0 || categorySummary.candidates !== 35) errors.push(`${category.id}: expected 65 published and 35 candidates`);
}
...
if (queue.totals.existing !== 910 || queue.totals.candidates !== 490 || queue.totals.published !== 910 || queue.totals.sourceRemediation !== 0 || queue.totals.claimReviewQueued !== 0 || queue.totals.researchQueued !== 490) errors.push('W14 totals must be 910 published existing and 490 queued candidates');
if (errors.length) {
  console.error(`W14 production queue validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W14 production queue validation: 1,400 topics; 910 published, 490 candidates, 65 per category');
