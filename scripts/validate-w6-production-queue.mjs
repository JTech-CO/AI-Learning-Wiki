...
const [schema, queue, topics, batches, evidence, categories, w4, w5, w6] = await Promise.all([
...
const readyIds = new Set([...w4.articles, ...w5.articles, ...w6.articles]
...
if (readyIds.size !== 42) errors.push(`expected 42 unique publication-ready articles, found ${readyIds.size}`);
...
for (const categoryId of categoryIds) {
  ...
  if (!summary || summary.topics !== 100 || summary.published !== 3 || summary.published + summary.existingQueued + summary.candidates !== 100) errors.push(`${categoryId}: category production counts are invalid`);
}
...
if (queue.totals.sourceRemediation + queue.totals.claimReviewQueued !== 108) errors.push('the 108 remaining existing articles must stay in explicit review stages');

if (errors.length) {
  console.error(`W6 production queue validation: ${errors.length} error(s)\n${errors.slice(0, 100).join('\n')}`);
  process.exit(1);
}
console.log(`W6 production queue validation: 1,400 topics in 56 batches; 42 published, 108 existing queued, 1,250 candidate research topics`);
