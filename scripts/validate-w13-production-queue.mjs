...
const [schema, queue, topics, batches, evidence, categories, ...ledgers] = await Promise.all([
  ...
  ...['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)),
]);
const errors = [];
...
if (queueById.size !== 1400 || topicById.size !== 1400) errors.push('W13 queue must contain 1,400 unique topics');
if (readyIds.size !== 770) errors.push(`expected 770 unique ready articles, found ${readyIds.size}`);
for (const [topicId, topic] of topicById) {
  ...
  if (topic.state === 'existing' && !item.publicationReady) errors.push(`${topicId}: existing article is not publication-ready after W13`);
}
...
for (const category of categories.categories) {
  ...
  if (!categorySummary || categorySummary.topics !== 100 || categorySummary.published !== 55 || categorySummary.existingQueued !== 0 || categorySummary.candidates !== 45) errors.push(`${category.id}: expected 55 published and 45 candidates`);
}
...
if (queue.totals.existing !== 770 || queue.totals.candidates !== 630 || queue.totals.published !== 770 || queue.totals.sourceRemediation !== 0 || queue.totals.claimReviewQueued !== 0 || queue.totals.researchQueued !== 630) errors.push('W13 totals must be 770 published existing and 630 queued candidates');
if (errors.length) {
  console.error(`W13 production queue validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W13 production queue validation: 1,400 topics; 770 published, 630 candidates, 55 per category');
