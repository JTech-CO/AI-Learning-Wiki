...
import { familyForW17 } from './w17-content-lib.mjs';
import { topicFacts } from './w17-topic-facts.mjs';

const readJson = async (file) => JSON.parse(await readFile(file, 'utf8'));
const sha = (value) => createHash('sha256').update(value).digest('hex');
const segmenter = new Intl.Segmenter('ko', { granularity: 'sentence' });
const claimsOf = (body) => [...segmenter.segment(body)].map(({ segment }) => segment.trim()).filter(Boolean);
const priorMilestones = ['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14', 'w15', 'w16'];
const [schema, ledger, rules, summary, manifest, w2, verification, categories, ...priorLedgers] = await Promise.all([
  ...
  readJson('content-model/evidence/w17-review-rules.json'),
  readJson('content-model/evidence/w17-summary.json'),
  readJson('content-model/evidence/w17-batch-manifest.json'),
...
const rulesText = await readFile('content-model/evidence/w17-review-rules.json', 'utf8');
if (ledger.rulesSha256 !== sha(rulesText)) errors.push('rulesSha256 differs from W17 review rules');
...
for (const reviewed of ledger.articles) {
  ...
  if (!rule || !manifestItem) errors.push(`${article.id}: W17 rule or manifest item missing`);
  ...
  if (rule?.priorStage !== 'research-queued' || manifestItem?.previousStage !== 'research-queued' || manifestItem?.previousState !== 'candidate') errors.push(`${article.id}: did not enter W17 from the candidate research gate`);
  if (!w2ById.get(article.id)?.audit.readyForManualClaimReview) errors.push(`${article.id}: W2 evidence gate not met`);
  if (article.reviewedAt !== rules.reviewedAt || article.status !== 'reviewed') errors.push(`${article.id}: review metadata differs from W17 rules`);
  ...
  const families = new Set(article.sources.map(familyForW17));
...
if (articleCount < 1330) errors.push(`expected at least 1330 article files after W17, found ${articleCount}`);
if (ledger.articles.length !== 140 || rules.articles.length !== 140 || manifest.topics.length !== 140) errors.push('W17 must contain exactly 140 new articles');
if (reviewedIds.size !== 140) errors.push('W17 reviewed article IDs are not unique');
for (const category of categories.categories) if (reviewedCategories.get(category.id) !== 10) errors.push(`${category.id}: expected 10 W17 articles`);
if (manifest.totals.newlyCreated !== 140 || manifest.totals.cumulativeArticles !== 1330 || manifest.slices.length !== 10 || manifest.slices.some((slice) => slice.topics.length !== 14)) errors.push('W17 batch manifest totals are invalid');
if (summary.batchReviewedArticles !== 140 || summary.cumulativeReviewedArticles !== 1330 || summary.publicationReadyArticles !== 140 || summary.newlyCreatedArticles !== 140 || summary.existingArticles !== 1330 || summary.candidateTopics !== 70 || summary.topicsPerCategory !== 95 || !summary.bodyHashLocked) errors.push('W17 summary is incomplete');
if (summary.reviewedClaimUnits !== claimCount || summary.acceptedClaimUnits !== claimCount) errors.push('W17 summary claim counts differ from ledger');
if (verification.totals.unavailable !== 0 || verification.totals.restricted !== 0) errors.push('W17 source verification contains unavailable or restricted URLs');

if (errors.length) {
  console.error(`W17 claim validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log(`W17 claim validation: 140 new articles, ${claimCount} locked claims, 1330 publication-ready cumulatively`);
