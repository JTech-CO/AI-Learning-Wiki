...
const [schema, queue, topics, batches, evidence, categories, ...ledgers] = await Promise.all([
  ...
  ...['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14', 'w15', 'w16', 'w17'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)),
]);
const errors = [];
...
if (queueById.size !== 1400 || topicById.size !== 1400) errors.push('W17 queue must contain 1,400 unique topics');
if (readyIds.size !== 1330) errors.push(`expected 1330 unique ready articles, found ${readyIds.size}`);
for (const [topicId, topic] of topicById) {
  ...
  if (item.state === 'existing' && !item.publicationReady) errors.push(`${topicId}: existing article is not publication-ready after W17`);
}
...
for (const category of categories.categories) {
  ...
  if (!categorySummary || categorySummary.topics !== 100 || categorySummary.published !== 95 || categorySummary.existingQueued !== 0 || categorySummary.candidates !== 5) errors.push(`${category.id}: expected 95 published and 5 candidates`);
}
...
if (queue.totals.existing !== 1330 || queue.totals.candidates !== 70 || queue.totals.published !== 1330 || queue.totals.sourceRemediation !== 0 || queue.totals.claimReviewQueued !== 0 || queue.totals.researchQueued !== 70) errors.push('W17 totals must be 1330 published existing and 70 queued candidates');
if (errors.length) {
  console.error(`W17 production queue validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W17 production queue validation: 1,400 topics; 1330 published, 70 candidates, 95 per category');
