...
const mode = argv.includes('--release') ? 'release' : 'prepare';
const dryRun = argv.includes('--dry-run');
const manifest = loadEditorQualityManifest();
const buildClock = resolveBuildClock({ argv, manifest });
const { asOf } = buildClock;
const asOfDate = new Date(`${asOf}T00:00:00.000Z`);
const day = 24 * 60 * 60 * 1000;

for (const baseline of verifyFrozenBaselines(manifest)) {
  if (!baseline.intact) throw new Error(`${baseline.id} frozen baseline hash changed: ${baseline.path}`);
}

const policyPath = 'content-model/taxonomy/quality-policy.json';
const taxonomyPath = 'content-model/taxonomy/categories.json';
const articleSchemaPath = 'content-model/schema.article.json';
const wikiPath = 'public/data/wiki-index.json';
const promptPath = 'public/data/prompts.json';
const artifactPath = 'public/data/snippets.json';
const sourceVerificationPath = 'content-model/evidence/source-verification.json';
const registryPath = 'content-model/labs/registry.json';

const policy = readJson(policyPath);
const taxonomy = readJson(taxonomyPath);
const articleSchema = readJson(articleSchemaPath);
...
const priorityCounts = { blocking: 0, attention: 0, scheduled: 0 };
...
const articleAudit = articles.map((article) => {
  ...
  const dueDate = addDays(reviewedDate, reviewDays[article.volatility]);
  ...
  const sectionIdDuplicates = article.sections.length - new Set(article.sections.map((section) => section.id)).size;
  const evidenceMissing = [];

  unresolvedRelations += unresolved.length;
  duplicateSectionIds += sectionIdDuplicates;
  ...
  for (const section of article.sections) {
    ...
    for (const ref of refs) {
      if (!Number.isInteger(ref) || ref < 1 || ref > article.sources.length) invalidSourceReferences += 1;
    }
  }
  ...
  } else reviewCounts.currentBeyond90Days += 1;
  ...
  const belowMinimumSections = article.sections.length < contract.minimumSectionCount;
  const reasons = [];
  ...
  if (article.sources.length < contract.minimumIndependentSourceFamilies) reasons.push('최소 출처 수 기준을 충족하지 않는다.');
  if (evidenceMissing.length > 0) reasons.push('근거 참조가 없는 본문 섹션이 있다.');
  if (belowMinimumSections) reasons.push('문서 스키마의 최소 섹션 수를 충족하지 않는다.');
  if (sectionIdDuplicates > 0) reasons.push('중복된 본문 섹션 ID가 있다.');
  ...
  if (
    ...
    || article.sources.length < contract.minimumIndependentSourceFamilies
    || evidenceMissing.length > 0
    || belowMinimumSections
    || sectionIdDuplicates > 0
    || unresolved.length > 0
  ) priority = 'blocking';
...
const untrackedSourceUrls = [...articleSourceUrls].filter((url) => !sourceByUrl.has(url));
...
const articlesMeetingSourceMinimum = articles.filter(
  (article) => article.sources.length >= contract.minimumIndependentSourceFamilies,
).length;
const reviewedArticles = articles.filter(
  (article) => !contract.publicationStatus || article.status === contract.publicationStatus,
).length;
const validSectionCounts = articles.filter((article) => article.sections.length >= contract.minimumSectionCount).length;
const belowMinimumSections = articles.length - validSectionCounts;
const belowRecommendedSections = articles.filter(
  (article) => article.sections.length < contract.recommendedSectionCount.min,
).length;
const aboveRecommendedSections = articles.filter(
  (article) => article.sections.length > contract.recommendedSectionCount.max,
).length;
...
const categories = taxonomy.categories.map((category) => {
  ...
  const audits = members.map((article) => auditById.get(article.id));
  return {
    id: category.id,
    title: category.title,
    articles: members.length,
    overdue: audits.filter((item) => item.reviewBand === 'overdue').length,
    dueWithin90Days: audits.filter((item) => ['due-30', 'due-90'].includes(item.reviewBand)).length,
    sourceWatch: audits.filter((item) => item.sourceIssues > 0).length,
  };
});

const unresolvedCourseSteps = courses.flatMap((course) => course.steps).filter((step) => !articleIds.has(step.ref));
...
const analyzeDist = () => {
  ...
  for (const { full, relative } of htmlFiles) {
    ...
    const route = `/${relative.replace(/index\.html$/u, '')}`.replace(/\/{2,}/gu, '/');
    const hrefs = [...html.matchAll(/\bhref=["']([^"']+)["']/giu)].map((match) => match[1].replaceAll('&amp;', '&'));
    for (const href of new Set(hrefs)) {
      if (/^(?:[a-z]+:|\/\/|#)/iu.test(href) || href.includes('${')) continue;
  ...
  const routeStatus = Object.fromEntries(requiredRoutes.map((route) => [route, outputFiles.has(route)]));
  return {
    status: broken.length === 0 && Object.values(routeStatus).every(Boolean) ? 'complete' : 'failed',
    htmlPages: htmlFiles.length,
    internalLinkReferences,
    brokenInternalLinks: broken.length,
    requiredRoutes: routeStatus,
  };
};
...
const contentGates = [
  {
    ...
    description: `공개 문서 상태가 모두 ${contract.publicationStatus ?? '정책 허용 상태'}인지 확인한다.`,
  },
  {
    id: 'review-cadence',
    label: '재검토 기한 준수',
    ...
    description: `향후 90일 안에 ${(reviewCounts.dueWithin30Days + reviewCounts.dueWithin90Days).toLocaleString('ko-KR')}개를 계획 검토한다.`,
  },
  {
    ...
    description: `문서당 최소 ${contract.minimumIndependentSourceFamilies.toLocaleString('ko-KR')}개 출처와 본문 섹션의 유효한 sourceRefs를 확인한다.`,
  },
  {
    id: 'source-reachability',
    label: '외부 출처 도달성',
    status: sourceStateCount('unavailable') === 0 && untrackedSourceUrls.length === 0 ? 'pass' : 'blocking',
    value: `도달 ${sourceStateCount('reachable').toLocaleString('ko-KR')} · 제한 ${sourceStateCount('restricted').toLocaleString('ko-KR')}`,
  ...
  },
  {
    ...
    description: `최소 ${contract.minimumSectionCount.toLocaleString('ko-KR')}개 섹션과 고유한 섹션 ID, 선수·관련 문서 및 공개 색인의 연결을 확인한다.`,
  },
  {
...
const staticBuildGate = {
  ...
  value: release.status === 'pending' ? '빌드 후 확정' : `${release.htmlPages.toLocaleString('ko-KR')} 페이지`,
...
const snapshot = {
  schemaVersion: manifest.snapshot.schemaVersion,
  snapshotId: manifest.snapshot.id,
  generatedAt: buildClock.generatedAt,
  asOf,
  formulaVersion: manifest.snapshot.formulaVersion,
  ...
  evidence: {
    ...
    reachableSourceUrls: sourceStateCount('reachable'),
    restrictedSourceUrls: sourceStateCount('restricted'),
    unavailableSourceUrls: sourceStateCount('unavailable'),
  ...
  structure: {
    minimumSectionCount: contract.minimumSectionCount,
    recommendedSectionCountMin: contract.recommendedSectionCount.min,
    recommendedSectionCountMax: contract.recommendedSectionCount.max,
    validSectionCounts,
    belowMinimumSections,
    belowRecommendedSections,
    aboveRecommendedSections,
    duplicateSectionIds,
    unresolvedRelations,
    orphanArticles,
    publicIndexMatches,
  ...
  release,
  provenance: {
    buildClockSource: buildClock.source,
    ...
    inputsSha256: {
      ...
      articleSchema: sha256(readText(articleSchemaPath)),
      qualityPolicy: sha256(readText(policyPath)),
      taxonomy: sha256(readText(taxonomyPath)),
      wikiIndex: sha256(readText(wikiPath)),
      sourceVerification: sha256(readText(sourceVerificationPath)),
      promptLibrary: sha256(readText(promptPath)),
      artifactLibrary: sha256(readText(artifactPath)),
    },
  },
};
...
if (!dryRun) {
  for (const output of [manifest.outputs.canonical, manifest.outputs.public]) {
    fs.mkdirSync(path.dirname(output), { recursive: true });
    fs.writeFileSync(output, serialized);
  }
  if (mode === 'release' && fs.existsSync(path.dirname(manifest.outputs.release))) {
    fs.writeFileSync(manifest.outputs.release, serialized);
  }
}

console.log(
  `Current editor quality (${mode}${dryRun ? ', dry-run' : ''}, ${asOf}): `
  + `${articles.length} articles, ${queue.length} queued, `
