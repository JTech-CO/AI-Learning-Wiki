...
import { familyForW16 } from './w16-content-lib.mjs';
...
const priorMilestones = ['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14', 'w15'];
const [schema, ledger, rules, summary, manifest, w2, verification, categories, ...priorLedgers] = await Promise.all([
  ...
  readJson('content-model/evidence/w16-review-rules.json'),
  readJson('content-model/evidence/w16-summary.json'),
  readJson('content-model/evidence/w16-batch-manifest.json'),
...
const rulesText = await readFile('content-model/evidence/w16-review-rules.json', 'utf8');
if (ledger.rulesSha256 !== sha(rulesText)) errors.push('rulesSha256 differs from W16 review rules');
...
for (const reviewed of ledger.articles) {
  ...
  if (!rule || !manifestItem) errors.push(`${article.id}: W16 rule or manifest item missing`);
  ...
  if (rule?.priorStage !== 'research-queued' || manifestItem?.previousStage !== 'research-queued' || manifestItem?.previousState !== 'candidate') errors.push(`${article.id}: did not enter W16 from the candidate research gate`);
  if (!w2ById.get(article.id)?.audit.readyForManualClaimReview) errors.push(`${article.id}: W2 evidence gate not met`);
  if (article.reviewedAt !== rules.reviewedAt || article.status !== 'reviewed') errors.push(`${article.id}: review metadata differs from W16 rules`);
  ...
  const families = new Set(article.sources.map(familyForW16));
...
if (articleCount < 1190) errors.push(`expected at least 1190 article files after W16, found ${articleCount}`);
if (ledger.articles.length !== 140 || rules.articles.length !== 140 || manifest.topics.length !== 140) errors.push('W16 must contain exactly 140 new articles');
if (reviewedIds.size !== 140) errors.push('W16 reviewed article IDs are not unique');
for (const category of categories.categories) if (reviewedCategories.get(category.id) !== 10) errors.push(`${category.id}: expected 10 W16 articles`);
if (manifest.totals.newlyCreated !== 140 || manifest.totals.cumulativeArticles !== 1190 || manifest.slices.length !== 10 || manifest.slices.some((slice) => slice.topics.length !== 14)) errors.push('W16 batch manifest totals are invalid');
if (summary.batchReviewedArticles !== 140 || summary.cumulativeReviewedArticles !== 1190 || summary.publicationReadyArticles !== 140 || summary.newlyCreatedArticles !== 140 || summary.existingArticles !== 1190 || summary.candidateTopics !== 210 || summary.topicsPerCategory !== 85 || !summary.bodyHashLocked) errors.push('W16 summary is incomplete');
if (summary.reviewedClaimUnits !== claimCount || summary.acceptedClaimUnits !== claimCount) errors.push('W16 summary claim counts differ from ledger');
if (verification.totals.unavailable !== 0 || verification.totals.restricted !== 0) errors.push('W16 source verification contains unavailable or restricted URLs');

if (errors.length) {
  console.error(`W16 claim validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log(`W16 claim validation: 140 new articles, ${claimCount} locked claims, 1190 publication-ready cumulatively`);
