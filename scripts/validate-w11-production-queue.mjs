...
const [schema, queue, topics, batches, evidence, categories, ...ledgers] = await Promise.all([
  ...
  ...['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)),
]);
const errors = [];
...
if (queueById.size !== 1400 || topicById.size !== 1400) errors.push('W11 queue must contain 1,400 unique topics');
if (readyIds.size !== 490) errors.push(`expected 490 unique ready articles, found ${readyIds.size}`);
for (const [topicId, topic] of topicById) {
  ...
  if (topic.state === 'existing' && !item.publicationReady) errors.push(`${topicId}: existing article is not publication-ready after W11`);
}
...
for (const category of categories.categories) {
  ...
  if (!categorySummary || categorySummary.topics !== 100 || categorySummary.published !== 35 || categorySummary.existingQueued !== 0 || categorySummary.candidates !== 65) errors.push(`${category.id}: expected 35 published and 65 candidates`);
}
...
if (queue.totals.existing !== 490 || queue.totals.candidates !== 910 || queue.totals.published !== 490 || queue.totals.sourceRemediation !== 0 || queue.totals.claimReviewQueued !== 0 || queue.totals.researchQueued !== 910) errors.push('W11 totals must be 490 published existing and 910 queued candidates');
if (errors.length) {
  console.error(`W11 production queue validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W11 production queue validation: 1,400 topics; 490 published, 910 candidates, 35 per category');
