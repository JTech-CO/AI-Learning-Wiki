...
const [schema, queue, topics, batches, evidence, categories, ...ledgers] = await Promise.all([
  ...
  ...['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14', 'w15'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)),
]);
const errors = [];
...
if (queueById.size !== 1400 || topicById.size !== 1400) errors.push('W15 queue must contain 1,400 unique topics');
if (readyIds.size !== 1050) errors.push(`expected 1050 unique ready articles, found ${readyIds.size}`);
for (const [topicId, topic] of topicById) {
  ...
  if (item.state === 'existing' && !item.publicationReady) errors.push(`${topicId}: existing article is not publication-ready after W15`);
}
...
for (const category of categories.categories) {
  ...
  if (!categorySummary || categorySummary.topics !== 100 || categorySummary.published !== 75 || categorySummary.existingQueued !== 0 || categorySummary.candidates !== 25) errors.push(`${category.id}: expected 75 published and 25 candidates`);
}
...
if (queue.totals.existing !== 1050 || queue.totals.candidates !== 350 || queue.totals.published !== 1050 || queue.totals.sourceRemediation !== 0 || queue.totals.claimReviewQueued !== 0 || queue.totals.researchQueued !== 350) errors.push('W15 totals must be 1050 published existing and 350 queued candidates');
if (errors.length) {
  console.error(`W15 production queue validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W15 production queue validation: 1,400 topics; 1050 published, 350 candidates, 75 per category');
