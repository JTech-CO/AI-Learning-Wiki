...
const [baselineAudit, baselineDepth, report, storedAudit, storedQueue, storedDepth, policy, languageAudit, terminologyAudit, bilingualTitleAudit, freshnessAudit, inputs] = await Promise.all([
  ...
  readJson('content-model/quality/bilingual-title-normalization.json'),
  readJson('content-model/evidence/content-freshness-audit.json'),
  loadW19Inputs(),
]);
...
const bilingualTitleAuditIds = new Set(bilingualTitleAudit.changes.map((change) => change.articleId));
const freshnessAuditById = new Map(freshnessAudit.articles.map((change) => [change.articleId, change]));
const freshnessAuditIds = new Set(freshnessAuditById.keys());
assert.equal(languageAuditIds.size, 94);
assert.equal(terminologyAuditIds.size, 58);
assert.equal(bilingualTitleAudit.totals.articlesScanned, 1600);
assert.equal(bilingualTitleAuditIds.size, 46);
assert.equal(bilingualTitleAudit.totals.occurrencesRemoved, 552);
assert.equal(bilingualTitleAudit.changes.reduce((sum, change) => sum + change.occurrencesRemoved, 0), 552);
assert.equal(bilingualTitleAudit.totals.particleCorrectionArticles, 14);
assert.equal(bilingualTitleAudit.totals.particleCorrections, 56);
assert.equal(freshnessAudit.version, 'content-freshness-audit-v1');
assert.equal(freshnessAudit.reviewedAt, '2026-07-23');
assert.equal(freshnessAuditIds.size, 4);
...
for (const { article, raw } of inputs.loaded) {
  ...
  const auditedChange = reportById.has(article.id) || languageAuditIds.has(article.id) || terminologyAuditIds.has(article.id) || bilingualTitleAuditIds.has(article.id) || freshnessAuditIds.has(article.id);
  ...
  if (freshnessAuditById.has(article.id)) assert.equal(freshnessAuditById.get(article.id).afterSha256, sha256(raw), `${article.id}: freshness audit hash mismatch`);
}
