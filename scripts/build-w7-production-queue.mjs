...
const [topics, batches, evidence, research, w4, w5, w6, w7, w7Summary] = await Promise.all([
...
const publicationReady = new Set([...w4.articles, ...w5.articles, ...w6.articles, ...w7.articles]
...
const queue = {
  version: 'W7-2026-07-13',
  generatedAt: w7Summary.generatedAt,
...
await writeFile('content-model/evidence/w7-production-queue.json', `${JSON.stringify(queue, null, 2)}\n`, 'utf8');
console.log(`W7 production queue: ${queue.totals.topics} topics; ${queue.totals.published} published, ${queue.totals.sourceRemediation} source remediation, ${queue.totals.claimReviewQueued} claim review, ${queue.totals.researchQueued} research queued`);
