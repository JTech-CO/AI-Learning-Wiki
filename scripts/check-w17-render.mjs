...
const ledgers = await Promise.all(['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14', 'w15', 'w16', 'w17'].map(async (milestone) => JSON.parse(await readFile(`content-model/evidence/${milestone}-claim-ledger.json`, 'utf8'))));
...
for (const articleId of articleDirs) {
  ...
  if (w17.articles.some((article) => article.articleId === articleId)) {
...
if (readyPages !== 1330) errors.push(`expected 1330 rendered ready pages, found ${readyPages}`);
if (errors.length) {
  console.error(`W17 rendered status check: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W17 rendered status check: exactly 1330 publication-ready pages are labeled and W17 evidence anchors resolve');
