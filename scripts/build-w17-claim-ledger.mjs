...
const priorLedgers = await Promise.all(['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14', 'w15', 'w16'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)));
...
const summary = {
  ...
  topicsPerCategory: 95,
  nextGate: 'W18에서 분야별 마지막 우선순위 신규 문서 5개씩 검토',
};

await Promise.all([
  writeFile('content-model/evidence/w17-claim-ledger.json', `${JSON.stringify(ledger, null, 2)}\n`, 'utf8'),
  writeFile('content-model/evidence/w17-summary.json', `${JSON.stringify(summary, null, 2)}\n`, 'utf8'),
]);
console.log(`W17 claim ledger: ${articles.length} articles, ${summary.reviewedSections} sections, ${claims.length} sentence claims locked; ${summary.cumulativeReviewedArticles} cumulative`);
