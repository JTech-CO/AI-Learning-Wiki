...
const previousReady = [...w4.articles, ...w5.articles].filter((article) => article.publicationReady).length;
const summary = {
  ...
  nextGate: '분야별 100개, 총 1,400개 제작 큐에서 출처 보강과 문장 단위 검토를 순차 실행',
};
await writeFile('content-model/evidence/w6-claim-ledger.json', `${JSON.stringify(ledger, null, 2)}\n`, 'utf8');
await writeFile('content-model/evidence/w6-summary.json', `${JSON.stringify(summary, null, 2)}\n`, 'utf8');
console.log(`W6 claim ledger: ${articles.length} articles, ${summary.reviewedSections} sections, ${claims.length} sentence claims locked; ${summary.cumulativeReviewedArticles} cumulative`);
