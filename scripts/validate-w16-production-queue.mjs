...
const [schema, queue, topics, batches, evidence, categories, ...ledgers] = await Promise.all([
  ...
  ...['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14', 'w15', 'w16'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)),
]);
const errors = [];
...
if (queueById.size !== 1400 || topicById.size !== 1400) errors.push('W16 queue must contain 1,400 unique topics');
if (readyIds.size !== 1190) errors.push(`expected 1190 unique ready articles, found ${readyIds.size}`);
for (const [topicId, topic] of topicById) {
  ...
  if (item.state === 'existing' && !item.publicationReady) errors.push(`${topicId}: existing article is not publication-ready after W16`);
}
...
for (const category of categories.categories) {
  ...
  if (!categorySummary || categorySummary.topics !== 100 || categorySummary.published !== 85 || categorySummary.existingQueued !== 0 || categorySummary.candidates !== 15) errors.push(`${category.id}: expected 85 published and 15 candidates`);
}
...
if (queue.totals.existing !== 1190 || queue.totals.candidates !== 210 || queue.totals.published !== 1190 || queue.totals.sourceRemediation !== 0 || queue.totals.claimReviewQueued !== 0 || queue.totals.researchQueued !== 210) errors.push('W16 totals must be 1190 published existing and 210 queued candidates');
if (errors.length) {
  console.error(`W16 production queue validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W16 production queue validation: 1,400 topics; 1190 published, 210 candidates, 85 per category');
