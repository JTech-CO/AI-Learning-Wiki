...
const [w4, w5, w6, w7, priorQueue] = await Promise.all([
...
const previousReady = [...w4.articles, ...w5.articles, ...w6.articles, ...w7.articles].filter((article) => article.publicationReady).length;
...
const summary = {
  ...
  nextGate: 'W8 제작 큐에서 분야별 다음 우선 문서를 같은 근거 게이트로 순차 확장',
};
await writeFile('content-model/evidence/w8-claim-ledger.json', `${JSON.stringify(ledger, null, 2)}\n`, 'utf8');
await writeFile('content-model/evidence/w8-summary.json', `${JSON.stringify(summary, null, 2)}\n`, 'utf8');
console.log(`W8 claim ledger: ${articles.length} articles, ${summary.reviewedSections} sections, ${claims.length} sentence claims locked; ${summary.cumulativeReviewedArticles} cumulative`);
