...
const w4 = await readJson('content-model/evidence/w4-claim-ledger.json');
const articles = [];
...
const summary = {
  ...
  cumulativeReviewedArticles: w4.articles.filter((article) => article.publicationReady).length + articles.length,
  ...
  bodyHashLocked: true,
  nextGate: 'W6에서 남은 기존 백과 문서의 검토 완료 범위를 확대하고 신규 1,400개 제작 파이프라인을 연결'
};
await writeFile('content-model/evidence/w5-claim-ledger.json', `${JSON.stringify(ledger, null, 2)}\n`, 'utf8');
await writeFile('content-model/evidence/w5-summary.json', `${JSON.stringify(summary, null, 2)}\n`, 'utf8');
console.log(`W5 claim ledger: ${articles.length} articles, ${summary.reviewedSections} sections, ${claims.length} sentence claims locked; ${summary.cumulativeReviewedArticles} cumulative`);
