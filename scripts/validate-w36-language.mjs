...
const bannedLiteralTranslations = /판단 문턱|릴리스 문턱|합격 문턱|지연과 비용 문턱|출력 소비자|건조 실행|새 연산을 내|기능의 종료 경로|호환 배포|호출 흔적/u;
...
assert.equal(audit.scope.articleSourcesChanged, 93);
assert.equal(audit.scope.lockedLedgerArticlesUpdated, 93);
assert.equal(audit.scope.lockedClaimUnitsRehashed, 195);
assert.equal(audit.ledgers.length, 13);
assert.equal(new Set(audit.ledgers.flatMap((ledger) => ledger.articleIds)).size, 93);
...
console.log(`W36 language: ${articleFiles.length} sources checked; 93 articles normalized, including 170 security-flow phrases and contextual API terminology`);
