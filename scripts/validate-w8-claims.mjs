...
const familyFor = (url, type) => {
  ...
  if (type === 'paper') return `research-paper-${host}`;
  if (type === 'book') return `academic-book-${host}`;
  if (type === 'standard' || type === 'specification') return `technical-standard-${host}`;
  if (type === 'documentation') return `official-documentation-${host}`;
  return host.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
};

const [schema, ledger, rules, summary, w2, w4, w5, w6, w7, priorQueue, categories] = await Promise.all([
  readJson('content-model/schema.claim-ledger.json'),
  readJson('content-model/evidence/w8-claim-ledger.json'),
  readJson('content-model/evidence/w8-review-rules.json'),
...
const rulesText = await readFile('content-model/evidence/w8-review-rules.json', 'utf8');
...
const priorIds = new Set([...w4.articles, ...w5.articles, ...w6.articles, ...w7.articles].filter((item) => item.publicationReady).map((item) => item.articleId));
...
for (const reviewed of ledger.articles) {
  ...
  if (priorIds.has(article.id)) errors.push(`${article.id}: W8 batch overlaps W4-W7`);
  if (!prior || !['source-remediation', 'research-queued'].includes(prior.stage)) errors.push(`${article.id}: invalid prior production stage`);
  if (expectedNextByCategory.get(rule?.categoryId) !== article.id) errors.push(`${article.id}: not the next ranked W7 queue topic for ${rule?.categoryId}`);
  if (!rule || !article.categories.includes(rule.categoryId)) errors.push(`${article.id}: category assignment differs from W8 rules`);
  if (article.reviewedAt !== rules.reviewedAt) errors.push(`${article.id}: reviewedAt differs from W8 review date`);
...
if (errors.length) {
  console.error(`W8 claim validation: ${errors.length} error(s)\n${errors.slice(0, 100).join('\n')}`);
  process.exit(1);
}
console.log(`W8 claim validation: 14 second-batch articles, 126 sections, ${claimCount} locked sentence claims; 70 publication-ready cumulatively`);
