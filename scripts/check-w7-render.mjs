...
const ledgers = await Promise.all(['w4', 'w5', 'w6', 'w7'].map(async (milestone) => JSON.parse(await readFile(`content-model/evidence/${milestone}-claim-ledger.json`, 'utf8'))));
...
if (readyPages !== 56) errors.push(`expected 56 rendered publication-ready pages, found ${readyPages}`);
if (errors.length) {
  console.error(`W7 rendered status check: ${errors.length} error(s)\n${errors.join('\n')}`);
  process.exit(1);
}
console.log('W7 rendered status check: exactly 56 publication-ready pages are labeled');
