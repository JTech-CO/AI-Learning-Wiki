...
const [w4, w5, w6, w7, w8, priorQueue] = await Promise.all([
...
const previousReady = [...w4.articles, ...w5.articles, ...w6.articles, ...w7.articles, ...w8.articles].filter((article) => article.publicationReady).length;
const priorStages = rules.articles.map((rule) => priorById.get(rule.articleId)?.stage);
const newlyCreatedArticles = priorStages.filter((stage) => stage === 'research-queued').length;
const summary = {
  ...
  existingArticles: priorQueue.totals.existing + newlyCreatedArticles,
  candidateTopics: priorQueue.totals.candidates - newlyCreatedArticles,
  topicsPerCategory: 15,
  nextGate: 'W10에서 분야별 다음 우선 문서 10개를 순위대로 제작',
};
await Promise.all([
  writeFile('content-model/evidence/w9-claim-ledger.json', `${JSON.stringify(ledger, null, 2)}\n`, 'utf8'),
  writeFile('content-model/evidence/w9-summary.json', `${JSON.stringify(summary, null, 2)}\n`, 'utf8'),
]);
console.log(`W9 claim ledger: ${articles.length} articles, ${summary.reviewedSections} sections, ${claims.length} sentence claims locked; ${summary.cumulativeReviewedArticles} cumulative`);
