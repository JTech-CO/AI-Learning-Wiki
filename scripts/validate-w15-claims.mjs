...
import { familyForW15 } from './w15-content-lib.mjs';
import { topicFacts } from './w15-topic-facts.mjs';

const readJson = async (file) => JSON.parse(await readFile(file, 'utf8'));
const sha = (value) => createHash('sha256').update(value).digest('hex');
const segmenter = new Intl.Segmenter('ko', { granularity: 'sentence' });
const claimsOf = (body) => [...segmenter.segment(body)].map(({ segment }) => segment.trim()).filter(Boolean);
const priorMilestones = ['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14'];
const [schema, ledger, rules, summary, manifest, w2, verification, categories, ...priorLedgers] = await Promise.all([
  ...
  readJson('content-model/evidence/w15-review-rules.json'),
  readJson('content-model/evidence/w15-summary.json'),
  readJson('content-model/evidence/w15-batch-manifest.json'),
...
const rulesText = await readFile('content-model/evidence/w15-review-rules.json', 'utf8');
if (ledger.rulesSha256 !== sha(rulesText)) errors.push('rulesSha256 differs from W15 review rules');
...
for (const reviewed of ledger.articles) {
  ...
  if (!rule || !manifestItem) errors.push(`${article.id}: W15 rule or manifest item missing`);
  ...
  if (rule?.priorStage !== 'research-queued' || manifestItem?.previousStage !== 'research-queued' || manifestItem?.previousState !== 'candidate') errors.push(`${article.id}: did not enter W15 from the candidate research gate`);
  if (!w2ById.get(article.id)?.audit.readyForManualClaimReview) errors.push(`${article.id}: W2 evidence gate not met`);
  if (article.reviewedAt !== rules.reviewedAt || article.status !== 'reviewed') errors.push(`${article.id}: review metadata differs from W15 rules`);
  ...
  const families = new Set(article.sources.map(familyForW15));
...
if (articleCount < 1050) errors.push(`expected at least 1050 article files after W15, found ${articleCount}`);
if (ledger.articles.length !== 140 || rules.articles.length !== 140 || manifest.topics.length !== 140) errors.push('W15 must contain exactly 140 new articles');
if (reviewedIds.size !== 140) errors.push('W15 reviewed article IDs are not unique');
for (const category of categories.categories) if (reviewedCategories.get(category.id) !== 10) errors.push(`${category.id}: expected 10 W15 articles`);
if (manifest.totals.newlyCreated !== 140 || manifest.totals.cumulativeArticles !== 1050 || manifest.slices.length !== 10 || manifest.slices.some((slice) => slice.topics.length !== 14)) errors.push('W15 batch manifest totals are invalid');
if (summary.batchReviewedArticles !== 140 || summary.cumulativeReviewedArticles !== 1050 || summary.publicationReadyArticles !== 140 || summary.newlyCreatedArticles !== 140 || summary.existingArticles !== 1050 || summary.candidateTopics !== 350 || summary.topicsPerCategory !== 75 || !summary.bodyHashLocked) errors.push('W15 summary is incomplete');
if (summary.reviewedClaimUnits !== claimCount || summary.acceptedClaimUnits !== claimCount) errors.push('W15 summary claim counts differ from ledger');
if (verification.totals.unavailable !== 0 || verification.totals.restricted !== 0) errors.push('W15 source verification contains unavailable or restricted URLs');

if (errors.length) {
  console.error(`W15 claim validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log(`W15 claim validation: 140 new articles, ${claimCount} locked claims, 1050 publication-ready cumulatively`);
