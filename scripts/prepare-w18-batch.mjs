...
const targetQueueItems = selectW18Targets(queue);
...
for (const queueItem of targetQueueItems) {
  ...
  if (queueItem.state !== 'candidate' || queueItem.stage !== 'research-queued') errors.push(`${topic.id}: W18 target is not a queued candidate`);
  if (!topicFacts[topic.id]) errors.push(`${topic.id}: curated W18 fact profile missing`);
  const file = path.join('content-model', 'articles', `${topic.id}.article.json`);
  const sources = sourcesForW18(topic, research, wikimedia, registry);
  const families = new Set(sources.map(familyForW18));
  ...
  const article = createW18Article(topic, sources);
...
for (const categoryId of categoryIds) {
  ...
  if (members.length !== 5) errors.push(`${categoryId}: expected 5 W18 topics, found ${members.length}`);
}
if (categoryIds.length !== 14) errors.push(`expected 14 categories, found ${categoryIds.length}`);
if (targetQueueItems.length !== 70 || manifestTopics.length !== 70) errors.push(`expected 70 new W18 topics, found ${manifestTopics.length}`);
if (new Set(targetQueueItems.map((topic) => topic.topicId)).size !== 70) errors.push('W18 target IDs are not unique');
if (errors.length) throw new Error(`W18 preparation failed:\n${errors.join('\n')}`);

const slices = Array.from({ length: 5 }, (_, index) => ({
  id: `w18-slice-${String(index + 1).padStart(2, '0')}`,
...
const manifest = {
  ...
  policy: { categories: 14, newTopicsPerCategory: 5, topicsPerSlice: 14, slices: 5, candidateOrder: 'ascending-rank' },
  totals: { topics: 70, newlyCreated: 70, cumulativeArticles: 1400 },
  slices,
  topics: manifestTopics,
};
const rules = { version: VERSION, reviewedAt: REVIEWED_AT, reviewer: 'Codex W18 expansion review', policy, factualSectionIds: FACTUAL_SECTION_IDS, articles: reviewRules };

await mkdir('content-model/evidence', { recursive: true });
await Promise.all([
  writeFile('content-model/evidence/w18-batch-manifest.json', `${JSON.stringify(manifest, null, 2)}\n`, 'utf8'),
  writeFile('content-model/evidence/w18-review-rules.json', `${JSON.stringify(rules, null, 2)}\n`, 'utf8'),
]);
console.log(`W18 preparation: ${manifestTopics.length} new articles in ${slices.length} resumable slices; 1400 cumulative`);
