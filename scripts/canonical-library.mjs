...
export async function buildPromptLibrary(root = process.cwd()) {
  ...
  const prompts = promptValues.map((prompt) => {
    ...
    return {
      ...
      relatedWikiUrl,
      url: `/prompt-explorer/?id=${encodeURIComponent(prompt.id)}#prompt-${encodeURIComponent(prompt.id)}`,
      kind: prompt.kind,
  ...
  const snippets = artifactValues.map((artifact) => {
    ...
    return {
      ...
      courseUrl: course.url,
      relatedWikiUrl,
      url: `/snippet-explorer/?id=${encodeURIComponent(artifact.id)}#snippet-${encodeURIComponent(artifact.id)}`,
