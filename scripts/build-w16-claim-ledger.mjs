...
const priorLedgers = await Promise.all(['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12', 'w13', 'w14', 'w15'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)));
...
const summary = {
  ...
  topicsPerCategory: 85,
  nextGate: 'W17에서 분야별 다음 우선순위 신규 문서 10개씩 검토',
};

await Promise.all([
  writeFile('content-model/evidence/w16-claim-ledger.json', `${JSON.stringify(ledger, null, 2)}\n`, 'utf8'),
  writeFile('content-model/evidence/w16-summary.json', `${JSON.stringify(summary, null, 2)}\n`, 'utf8'),
]);
console.log(`W16 claim ledger: ${articles.length} articles, ${summary.reviewedSections} sections, ${claims.length} sentence claims locked; ${summary.cumulativeReviewedArticles} cumulative`);
