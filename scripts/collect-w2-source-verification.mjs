...
async function verify(url) {
  try {
    ...
    return {
      ...
      state: response.ok || (response.status >= 300 && response.status < 400) ? 'reachable' : restricted ? 'restricted' : 'unavailable',
...
await writeFile('content-model/evidence/source-verification.json', `${JSON.stringify(snapshot, null, 2)}\n`);
console.log(`W2 source verification: ${sources.length} URLs; ${snapshot.totals.reachable} reachable, ${snapshot.totals.restricted} restricted, ${snapshot.totals.unavailable} unavailable`);
