...
const [topics, batches, evidence, research, w4, w5, w6, w6Summary] = await Promise.all([
...
const publicationReady = new Set([...w4.articles, ...w5.articles, ...w6.articles]
...
const queueTopics = topics.topics.map((topic) => {
  ...
  return {
    ...
    state: topic.state,
    rank: topic.rank,
    batchId: batchByTopic.get(topic.id),
    stage,
    publicationReady: ready,
    evidenceReady,
    manualReviewRequired: Boolean(researchCard?.manualReviewRequired),
    blockers,
  };
});
...
const byCategory = Object.fromEntries(categoryIds.map((categoryId) => {
  ...
  return [categoryId, {
    ...
    published: members.filter((topic) => topic.publicationReady).length,
    existingQueued: members.filter((topic) => topic.state === 'existing' && !topic.publicationReady).length,
    candidates: members.filter((topic) => topic.state === 'candidate').length,
  }];
}));
...
const queueBatches = batches.batches.map((batch) => {
  ...
  return {
    ...
    published: members.filter((topic) => topic.publicationReady).length,
    queued: members.filter((topic) => !topic.publicationReady).length,
  };
});

const queue = {
  version: 'W6-2026-07-13',
  generatedAt: w6Summary.generatedAt,
  policy: {
    publicationRequiresClaimLedger: true,
    candidatePublicationAllowed: false,
    stageOrder: ['research-queued', 'source-remediation', 'claim-review-queued', 'published'],
  },
  totals: {
    topics: queueTopics.length,
    categories: categoryIds.length,
    batches: queueBatches.length,
    existing: queueTopics.filter((topic) => topic.state === 'existing').length,
    candidates: queueTopics.filter((topic) => topic.state === 'candidate').length,
    published: countStage('published'),
    sourceRemediation: countStage('source-remediation'),
    claimReviewQueued: countStage('claim-review-queued'),
    researchQueued: countStage('research-queued'),
  },
  byCategory,
...
await writeFile('content-model/evidence/w6-production-queue.json', `${JSON.stringify(queue, null, 2)}\n`, 'utf8');
console.log(`W6 production queue: ${queue.totals.topics} topics; ${queue.totals.published} published, ${queue.totals.sourceRemediation} source remediation, ${queue.totals.claimReviewQueued} claim review, ${queue.totals.researchQueued} research queued`);
