...
const priorLedgers = await Promise.all(['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)));
...
const summary = {
  ...
  topicsPerCategory: 45,
  nextGate: 'W13에서 분야별 다음 우선순위 신규 문서 10개씩 검토',
};

await Promise.all([
  writeFile('content-model/evidence/w12-claim-ledger.json', `${JSON.stringify(ledger, null, 2)}\n`, 'utf8'),
  writeFile('content-model/evidence/w12-summary.json', `${JSON.stringify(summary, null, 2)}\n`, 'utf8'),
]);
console.log(`W12 claim ledger: ${articles.length} articles, ${summary.reviewedSections} sections, ${claims.length} sentence claims locked; ${summary.cumulativeReviewedArticles} cumulative`);
