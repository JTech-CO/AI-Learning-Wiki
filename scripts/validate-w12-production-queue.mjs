...
const [schema, queue, topics, batches, evidence, categories, ...ledgers] = await Promise.all([
  ...
  ...['w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10', 'w11', 'w12'].map((milestone) => readJson(`content-model/evidence/${milestone}-claim-ledger.json`)),
]);
const errors = [];
...
if (queueById.size !== 1400 || topicById.size !== 1400) errors.push('W12 queue must contain 1,400 unique topics');
if (readyIds.size !== 630) errors.push(`expected 630 unique ready articles, found ${readyIds.size}`);
for (const [topicId, topic] of topicById) {
  ...
  if (topic.state === 'existing' && !item.publicationReady) errors.push(`${topicId}: existing article is not publication-ready after W12`);
}
...
for (const category of categories.categories) {
  ...
  if (!categorySummary || categorySummary.topics !== 100 || categorySummary.published !== 45 || categorySummary.existingQueued !== 0 || categorySummary.candidates !== 55) errors.push(`${category.id}: expected 45 published and 55 candidates`);
}
...
if (queue.totals.existing !== 630 || queue.totals.candidates !== 770 || queue.totals.published !== 630 || queue.totals.sourceRemediation !== 0 || queue.totals.claimReviewQueued !== 0 || queue.totals.researchQueued !== 770) errors.push('W12 totals must be 630 published existing and 770 queued candidates');
if (errors.length) {
  console.error(`W12 production queue validation: ${errors.length} error(s)\n${errors.slice(0, 200).join('\n')}`);
  process.exit(1);
}
console.log('W12 production queue validation: 1,400 topics; 630 published, 770 candidates, 45 per category');
