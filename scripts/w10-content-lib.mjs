...
export const VERSION = 'W10-2026-07-14';
...
export function selectW10Targets(queue) {
...
export function sourcesForW10(topic, research, wikimedia, registry) {
  ...
  const selected = dedupe([...(primarySources[topic.primaryCategory] ?? []), ...supporting]).filter((source) => source.type !== 'encyclopedia').slice(0, 7);
  return dedupe([...selected, ...encyclopedia]).slice(0, 8);
}

function buildBodies(topic) {
  const fact = topicFacts[topic.id];
  const domain = context[topic.primaryCategory];
  if (!fact || !domain) throw new Error(`${topic.id}: W10 fact profile or category context missing`);
...
export function createW10Article(topic, sources) {
...
export function familyForW10(source) {
